...
use camino::Utf8Path;
use ccbr_project::identity::compute_ccbr_project_id;
use ccbr_storage::atomic::atomic_write_text;
use ccbr_terminal::backend::{TerminalBackend, TerminalBackendSelection};
...
pub fn load_registry_by_project_id(
    ...
    load_registry_by_project_id_impl(
        ...
        |work_dir| compute_ccbr_project_id(work_dir),
        upsert_registry,
    )
}
...
mod tests {
    ...
    fn test_api_upsert_registry_round_trip() {
        ...
        record.insert("ccbr_session_id".into(), "sess-api".into());
        record.insert(
            "work_dir".into(),
