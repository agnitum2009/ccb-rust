...
use ccbr_agents::models::{AgentSpec, ProjectConfig, RuntimeMode};
use ccbr_completion::{
...
fn agent_spec() -> AgentSpec {
    AgentSpec {
        ...
        provider_profile: ccbr_agents::models::ProviderProfileSpec::default(),
...
fn reply_candidates_extracted_from_item() {
    ...
    let candidates = ccbr_completion::reply_candidates_from_item(&item);
...
fn anchored_session_stability_settles_after_window() {
    use ccbr_completion::AnchoredSessionStabilityDetector;
...
fn snapshot_store_round_trip() {
    ...
    let layout = ccbr_storage::paths::PathLayout::new(tmp.path().to_str().unwrap());
...
impl ccbr_completion::CompletionSource for VecSource {
