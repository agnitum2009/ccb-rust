...
use ccbr_provider_core::pathing::{session_filename_for_agent, session_filename_for_instance};
use ccbr_provider_core::registry::ProviderBackendRegistry;
use ccbr_provider_core::runtime_shared::{pane_title_marker, provider_start_parts};
use ccbr_terminal::TmuxBackend;
...
impl ProviderLauncher {
    pub fn new() -> Self {
        Self {
            backend_registry: ccbr_providers::build_default_backend_registry(),
        }
    }
    ...
    match provider.trim().to_lowercase().as_str() {
        ...
        "codex" | "claude" | "gemini" | "agy" | "droid" => {
            ...
            session_filename_for_agent(provider, agent_name)
                .ok()
                .map(|filename| ccbr_dir.join(filename))
        }
...
fn build_deepseek_launch(ctx: &LaunchContext) -> Result<LaunchResult, String> {
    ...
    let pane_title_marker =
        ccbr_provider_core::runtime_shared::pane_title_marker(ctx.project_id, ctx.agent_name);
...
fn build_kimi_launch(ctx: &LaunchContext) -> Result<LaunchResult, String> {
    ...
    let session_filename = ccbr_provider_core::pathing::session_filename_for_instance(
        ".kimi-session",
        Some(ctx.agent_name),
    );
...
fn build_mimo_launch(ctx: &LaunchContext) -> Result<LaunchResult, String> {
    use ccbr_providers::mimo::{build_session_payload, build_start_cmd, prepare_launch_context};
...
fn build_gemini_launch(ctx: &LaunchContext) -> Result<LaunchResult, String> {
    use ccbr_providers::providers::gemini::{build_gemini_start_cmd, GeminiStartCommand};
...
fn build_agy_launch(ctx: &LaunchContext) -> Result<LaunchResult, String> {
    use ccbr_providers::providers::agy::{build_agy_start_cmd, AgyStartCommand};
    build_simple_provider_launch(
        ctx,
        "agy",
        |spec, runtime_dir, launch_session_id, prepared_state| {
...
fn build_droid_launch(ctx: &LaunchContext) -> Result<LaunchResult, String> {
    use ccbr_providers::droid::launcher::{
...
fn apply_template(template: &str, command: &str) -> Result<String, String> {
    ccbr_provider_core::runtime_shared::apply_provider_command_template(command, Some(template))
