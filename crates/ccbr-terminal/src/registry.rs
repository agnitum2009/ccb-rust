...
const REGISTRY_PREFIX: &str = "ccbr-session-";
...
pub fn upsert_registry(record: &serde_json::Map<String, serde_json::Value>) -> bool {
    let session_id = match record.get("ccbr_session_id").and_then(|v| v.as_str()) {
        ...
        _ => {
            eprintln!("[DEBUG] Registry update skipped: missing ccbr_session_id");
            return false;
        }
    };
...
mod tests {
    ...
    fn test_registry_path_and_is_stale() {
        ...
        assert!(path.to_string_lossy().contains("ccbr-session-abc123.json"));
    ...
    fn test_upsert_registry() {
        let tmp = std::env::temp_dir().join(format!("ccbr-registry-test-{}", std::process::id()));
        let _ = std::fs::create_dir_all(&tmp);
        let work_dir = tmp.to_string_lossy().to_string();
        let mut record = serde_json::Map::new();
        record.insert("ccbr_session_id".to_string(), "sess-1".into());
