...
pub const RUNTIME_ROOT_RECORD_TYPE: &str = "ccbr_runtime_root";
pub const RUNTIME_ROOT_REF_RECORD_TYPE: &str = "ccbr_runtime_root_ref";
...
pub fn runtime_socket_root_candidates() -> Vec<Utf8PathBuf> {
    ...
    if let Ok(xdg) = env::var("XDG_RUNTIME_DIR") {
        ...
        if !trimmed.is_empty() {
            candidates.push(Utf8PathBuf::from(format!("{}/ccbr-runtime", trimmed)));
        }
    }
    candidates.push(Utf8PathBuf::from("/tmp/ccbr-runtime"));
    if let Ok(tmp) = env::var("TMPDIR") {
        if !tmp.trim().is_empty() {
            candidates.push(Utf8PathBuf::from(format!("{}/ccbr-runtime", tmp.trim())));
        }
    }
...
pub fn runtime_socket_root() -> Utf8PathBuf {
    ...
    Utf8PathBuf::from("/tmp/ccbr-runtime")
}
