...
{
    let instance =
        ccbr_provider_core::instance_resolution::named_agent_instance(agent_name, "codex");
...
fn delivery_pane_content(state: &HashMap<String, Value>) -> Option<String> {
    ...
    backend
        ...
        .ok()
        .map(|s| ccbr_terminal::TmuxBackend::strip_ansi(&s))
}
...
impl CodexCommunicator {
    ...
    pub fn wrap_turn_prompt(&self, message: &str, req_id: &str) -> String {
        ccbr_provider_core::protocol::wrap_codex_turn_prompt(message, req_id)
    }
}
