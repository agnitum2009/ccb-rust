...
{
    let instance =
        ccbr_provider_core::instance_resolution::named_agent_instance(agent_name, "gemini");
...
pub fn materialize_gemini_home_config(
    ...
    let source_root: PathBuf = source_home
        ...
        .unwrap_or_else(ccbr_provider_core::source_home::current_provider_source_home);
...
fn materialize_gemini_settings(
    ...
    if let Some(env) = projected.get_mut("env").and_then(|v| v.as_object_mut()) {
        let api_keys: std::collections::HashSet<String> =
            ccbr_provider_profiles::provider_api_env_keys("gemini");
...
fn materialize_gemini_memory(
    ...
    let ccbr_memory = project_root.join(".ccbr").join("ccbr_memory.md");
    if ccbr_memory.is_file() {
        if let Ok(text) = std::fs::read_to_string(&ccbr_memory) {
            parts.push(text);
        }
    }
    ...
    let sha = ccbr_provider_core::memory_projection::text_file_sha256(target.as_path());
