...
/// Ensure a session payload has `work_dir`, `work_dir_norm`, and `ccbr_project_id`.
...
fn assign_project_id(payload: &mut Map<String, Value>, work_dir: &Path) {
    ...
    if let Ok(id) = compute_ccbr_project_id(work_dir) {
        payload.insert("ccbr_project_id".to_string(), Value::String(id));
    }
}

fn compute_ccbr_project_id(work_dir: &Path) -> Result<String, String> {
...
mod tests {
    ...
    #[test]
    fn test_infer_work_dir_from_session_file_skips_ccbr_dir() {
    ...
    fn test_ensure_work_dir_fields_populates_missing_values() {
        ...
        assert!(payload.contains_key("ccbr_project_id"));
    }
}
