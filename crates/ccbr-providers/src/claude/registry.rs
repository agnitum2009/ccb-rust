...
pub(crate) fn find_claude_session_file(work_dir: &Path) -> Option<PathBuf> {
    ...
    let filename = ccbr_provider_core::pathing::session_filename_for_instance(
        CLAUDE_SESSION_FILENAME,
        instance.as_deref(),
    );
    let ccbr_dir = target_project.join(".ccbr");
    let candidate = ccbr_dir.join(&filename);
...
mod tests {
    ...
    fn test_registry_resolves_named_workspace_session_file() {
        ...
        let session_file = ccbr_dir.join(".claude-agent3-session");
        std::fs::write(
            &session_file,
            serde_json::json!({
                "active": true,
