...
use ccbr_provider_core::runtime_specs::provider_marker_prefix;
...
pub trait OpenCodeCommunicatorState {
    ...
    fn set_ccbr_session_id(&mut self, id: String);
...
mod tests {
    ...
    fn test_initialize_state_populates_runtime_fields() {
        ...
        let session_file = tmp.path().join(".ccbr").join(".opencode-session");
        std::fs::create_dir_all(session_file.parent().unwrap()).unwrap();
        let session_info = serde_json::json!({
            "ccbr_session_id": "ccbr-open-1",
        ...
        assert_eq!(comm.ccbr_session_id, "ccbr-open-1");
