...
use ccbr_mailbox::facade_recording::CompletionDecision;
use ccbr_mailbox::models::{DeliveryScope, JobRecord, JobStatus, MessageEnvelope, TargetKind};
use ccbr_mailbox::{InboundEventStore, MailboxStore};
...
fn test_full_message_lifecycle() {
    ...
    assert!(matches!(
        ...
        ccbr_mailbox::models::MessageState::Completed
    ));
...
fn test_notice_and_cancelled_terminal() {
    ...
    assert!(matches!(
        message.message_state,
        ccbr_mailbox::models::MessageState::Cancelled
    ));
}
...
fn test_record_retry_attempt_increments_queue_without_refreshing_mailbox() {
    ...
    assert_eq!(
        events[1].event_type,
        ccbr_mailbox::models::InboundEventType::TaskRequest
    );
    ...
    assert!(matches!(
        ...
        ccbr_mailbox::models::MailboxState::Blocked
    ));
}
