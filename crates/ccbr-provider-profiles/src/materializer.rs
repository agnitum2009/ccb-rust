...
fn migrate_legacy_codex_profile_runtime_home(
    ...
    if let MigrationResult::Session(file, payload) = prepared {
        ccbr_storage::atomic::atomic_write_json(&file, &payload)?;
    }
...
fn prepare_legacy_codex_session_authority(
    ...
    let session_file = layout.ccbr_dir().join(
        ccbr_provider_core::pathing::session_filename_for_agent("codex", name)
...
fn write_profile_record(
    ...
    let path = runtime_dir.join("provider-profile.json");
    ccbr_storage::atomic::atomic_write_json(&path, &profile.to_record())?;
    Ok(path)
}
...
mod tests {
    ...
    fn layout(tmp: &TempDir) -> ccbr_storage::paths::PathLayout {
        ccbr_storage::paths::PathLayout::new(utf8(tmp.path()))
    }
