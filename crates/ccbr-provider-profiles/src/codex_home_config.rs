...
use ccbr_memory::render_provider_home_memory;
use serde_json::json;
...
const CODEX_LEGACY_OWNED_SKILL_NAMES: &[&str] = &["ccbr_config", "ccbr-config"];
...
pub fn materialize_codex_home_config(
    ...
    let source_home: Utf8PathBuf = match source_home {
        ...
        None => Utf8PathBuf::from_path_buf(
            ccbr_provider_core::source_home::current_provider_source_home(),
        )
    ...
    ccbr_provider_core::memory_projection::record_memory_projection_event(
        &memory_result,
        "codex",
...
fn write_codex_api_authority_config(
    ...
    ccbr_storage::atomic::atomic_write_text(target, &text)?;
    Ok(())
}
...
fn append_managed_codex_feature_overrides(target: &Utf8Path) -> crate::Result<()> {
    ...
    ccbr_storage::atomic::atomic_write_text(
        target,
...
fn copy_inherited_tree(
    ...
    if !enabled {
        ccbr_provider_core::projected_assets::remove_projected_path(
            target.as_std_path(),
            label,
    ...
    if (target.exists() || target.is_symlink()) && !marker.is_file() {
        ...
        if !target.is_dir()
            || ccbr_provider_core::projected_assets::tree_content_fingerprint(target.as_std_path())
                != ccbr_provider_core::projected_assets::tree_content_fingerprint(
                    source.as_std_path(),
                )
        {
            repair_owned_codex_skill_entries(source, target)?;
    ...
    if ccbr_provider_core::projected_assets::copy_projected_tree_to_cache(
        source.as_std_path(),
        target.as_std_path(),
        label,
    )
...
fn route_inherited_tree(
    ...
    ccbr_provider_core::projected_assets::route_projected_tree(
        source.as_std_path(),
        target.as_std_path(),
        enabled,
        label,
...
fn sync_codex_plugin_projection(
    ...
    if let Some(ref bundle) = bundle_tree {
        ...
        {
            ccbr_provider_core::projected_assets::write_projected_marker(
                target_tree.as_std_path(),
                CODEX_PLUGIN_PROJECTION_LABEL,
                "symlink",
                bundle.as_std_path(),
        ...
        .map_err(|e| crate::ProfilesError::Validation(e.to_string()))?
        {
            ...
            #[cfg(unix)]
            {
                if std::os::unix::fs::symlink(bundle, &target_tree).is_ok() {
                    ...
                    projected = ccbr_provider_core::projected_assets::route_projected_tree(
                        bundle.as_std_path(),
                        target_tree.as_std_path(),
                        true,
                        CODEX_PLUGIN_PROJECTION_LABEL,
    ...
    if source_sha.is_file() {
        ...
        ccbr_storage::atomic::atomic_write_text(&target_sha, &format!("{}\n", sha))?;
    }
    Ok(())
}
...
fn codex_plugin_bundle_sha(source_tree: &Utf8Path, source_sha: &Utf8Path) -> Option<String> {
    ...
    Some(ccbr_provider_core::projected_assets::tree_content_fingerprint(source_tree.as_std_path()))
        .filter(|s| !s.is_empty())
}
...
fn plugin_projection_is_current(
    ...
    let source_fp =
        ccbr_provider_core::projected_assets::tree_content_fingerprint(source_tree.as_std_path());
    let target_fp =
        ccbr_provider_core::projected_assets::tree_content_fingerprint(target_tree.as_std_path());
...
fn install_codex_activity_hooks(
    ...
    ccbr_storage::atomic::atomic_write_text(&hooks_path, &text)?;
...
fn merge_codex_activity_hook_state(
    ...
    if payload.is_empty() && !existing_text.trim().is_empty() {
        ...
        ccbr_storage::atomic::atomic_write_text(target_config, &replaced)?;
        return Ok(());
    }
    ...
    ccbr_storage::atomic::atomic_write_text(target_config, &text)?;
    Ok(())
}
...
fn materialize_codex_memory(
    ...
    agent_name: Option<&str>,
    workspace_path: Option<&Utf8Path>,
) -> crate::Result<ccbr_provider_core::memory_projection::MemoryProjectionResult> {
    let target = target_home.join("AGENTS.md");
    if same_path(source_home, target_home) {
        return Ok(
            ccbr_provider_core::memory_projection::memory_projection_result(
                "skipped",
...
fn materialize_provider_memory_file(
    ...
    fs::create_dir_all(target.parent().unwrap_or(target.as_ref()))?;
    ccbr_storage::atomic::atomic_write_text(target, &content)?;

    let sha256 = ccbr_provider_core::memory_projection::text_file_sha256(target.as_std_path());
    Ok(
        ccbr_provider_core::memory_projection::memory_projection_result(
            "materialized",
...
mod tests {
    ...
    fn test_copy_inherited_tree_creates_projection() {
        ...
        ccbr_storage::atomic::atomic_write_text(&source.join("demo/SKILL.md"), "demo\n").unwrap();
