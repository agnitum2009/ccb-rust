...
mod tests {
        ...
        let service = TmuxPaneService::new(
            move |args: &[&str], _check: bool, _capture: bool| -> anyhow::Result<TmuxRunOutput> {
                if args == [
                    ...
                    "-t",
                    "%3",
                    "#{pane_id}\t#{pane_title}\t#{pane_dead}\t#{@ccb_agent}\t#{@ccb_project_id}",
                ] {
    ...
    fn test_pane_service_finds_unique_pane_by_user_options() {
            ...
            move |args: &[&str], _check: bool, _capture: bool| -> anyhow::Result<TmuxRunOutput> {
                if args
                    == [
                        ...
                        "-a",
                        "-F",
                        "#{pane_id}\t#{@ccb_agent}\t#{@ccb_project_id}",
                    ]
                {
        ...
        expected.insert("ccb_agent".to_string(), "agent1".to_string());
        expected.insert("ccb_project_id".to_string(), "proj-2".to_string());
        assert_eq!(
