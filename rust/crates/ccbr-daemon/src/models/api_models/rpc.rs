...
/// `ccbr-cli` `method`/`params` shape. This keeps the daemon compatible with
...
impl RpcRequest {
    pub fn from_json(raw: &str) -> Result<Self, String> {
        ...
        // Normalize ccbr-cli shape (`method`/`params`) to daemon shape (`op`/`request`).
...
/// default. When responding to `ccbr-cli` we also include a `result` field.
#[derive(Debug, Clone, Serialize, Deserialize)]
