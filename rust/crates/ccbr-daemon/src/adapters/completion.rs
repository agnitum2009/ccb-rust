//! Adapters between daemon API models and the `ccbr-completion` crate.

use ccbr_completion::models::{JobRecord as CompletionJobRecord, JobRequest, TargetKind};

/// Convert a daemon `JobRecord` into the `ccbr-completion` job record shape.
