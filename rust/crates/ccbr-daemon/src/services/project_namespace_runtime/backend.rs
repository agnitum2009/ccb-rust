...
use ccbr_terminal::placeholders::{pane_placeholder_argv, pane_placeholder_cmd};
use ccbr_terminal::readiness::{
...
use ccbr_terminal::{TerminalBackend as _, TmuxBackend, TmuxOutput};

use crate::DaemonError;
use crate::Result;

// Re-export tmux error types so the public API matches Python's `__all__`.
pub use ccbr_terminal::readiness::{TmuxCommandError, TmuxTransientServerUnavailable};
...
impl ccbr_terminal::layouts::TmuxLayoutBackend for Backend {
    fn get_current_pane_id(&self) -> anyhow::Result<String> {
        ccbr_terminal::layouts::TmuxLayoutBackend::get_current_pane_id(&self.tmux)
    }

    fn is_alive(&self, pane_id: &str) -> bool {
        ccbr_terminal::layouts::TmuxLayoutBackend::is_alive(&self.tmux, pane_id)
    }
    ...
    fn create_pane(
        ...
        ccbr_terminal::layouts::TmuxLayoutBackend::create_pane(
            &self.tmux,
            cmd,
            cwd,
            direction,
    ...
    fn split_pane(
        ...
        ccbr_terminal::layouts::TmuxLayoutBackend::split_pane(
            &self.tmux,
            parent_pane_id,
            direction,
            percent,
    ...
    fn set_pane_title(&self, pane_id: &str, title: &str) {
        ccbr_terminal::layouts::TmuxLayoutBackend::set_pane_title(&self.tmux, pane_id, title);
    }

    fn set_pane_user_option(&self, pane_id: &str, name: &str, value: &str) {
        ccbr_terminal::layouts::TmuxLayoutBackend::set_pane_user_option(
    ...
    fn set_pane_style(
        ...
        ccbr_terminal::layouts::TmuxLayoutBackend::set_pane_style(
            &self.tmux,
            pane_id,
            border_style,
            active_border_style,
    ...
    fn tmux_run(&self, args: &[&str], check: bool, capture: bool) -> anyhow::Result<String> {
        ccbr_terminal::layouts::TmuxLayoutBackend::tmux_run(&self.tmux, args, check, capture)
    }
}
...
mod tests {
    ...
    fn test_build_backend() {
        ...
        let backend = build_backend(&factory, "/tmp/ccbr-test.sock").unwrap();
        assert_eq!(backend.socket_path, "/tmp/ccbr-test.sock");
    ...
    fn test_real_session_lifecycle() {
        let factory = BackendFactory::default();
        let backend = build_backend(&factory, "/tmp/ccbr-test-real.sock").unwrap();
