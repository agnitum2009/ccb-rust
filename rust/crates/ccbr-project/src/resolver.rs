...
use crate::discovery::{
    ...
    find_workspace_binding, is_dangerous_project_root, load_workspace_binding, project_ccbr_dir,
...
pub fn bootstrap_project(
    ...
    let config_dir = project_ccbr_dir(&root);
...
fn ensure_bootstrap_project_config(
    ...
    let config_dir = project_ccbr_dir(project_root);
    std::fs::create_dir_all(&config_dir)?;
...
fn _project_context(
    ...
    Ok(ProjectContext {
        ...
        config_dir: project_ccbr_dir(root),
...
fn _nested_anchor_bootstrap_error(project_root: &Utf8Path, parent_root: &Utf8Path) -> String {
    format!(
        ...
        parent_anchor = project_ccbr_dir(parent_root),
        new_anchor = project_ccbr_dir(project_root)
    )
}
