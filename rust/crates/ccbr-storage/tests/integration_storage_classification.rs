...
use ccbr_storage_classification::{classify_provider_home, summarize_storage, StorageClass};
...
fn test_storage_classification_keeps_provider_authority_and_cache_separate() {
    ...
    #[cfg(unix)]
    {
        ...
        write(
            ...
            &serde_json::to_string(&serde_json::json!({
                "record_type": "ccbr_projected_asset",
    ...
    assert_eq!(
        records["ccbr_memory.md"]["storage_class"].as_str(),
        Some("user_content")
    );
    assert_eq!(
        records["ccbr_memory.md"]["reason"].as_str(),
...
fn test_path_layout_ensures_provider_shared_cache_manifest() {
    ...
    assert_eq!(
        ...
        Some("ccbr_shared_cache_manifest")
    );
...
fn test_path_layout_ensures_provider_external_cache_manifest() {
    ...
    assert_eq!(
        manifest["record_type"].as_str(),
        Some("ccbr_external_provider_cache_manifest")
    );
    assert_eq!(manifest["provider"].as_str(), Some("claude"));
    assert_eq!(manifest["project_id"].as_str(), Some(layout.project_id()));
    assert_eq!(
