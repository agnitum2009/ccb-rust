...
use ccbr_agents::models::{AgentSpec, ProjectConfig, WorkspaceMode};
use ccbr_agents::store::AgentSpecStore;
use ccbr_project::identity::compute_project_id;
use ccbr_project::resolver::ProjectContext;
...
fn load_current_config_specs(project_root: &Path) -> Result<Vec<AgentSpec>> {
    ...
    let result =
        ccbr_agents::config::load_project_config(&layout).map_err(crate::WorkspaceError::Agents)?;
...
mod tests {
    use super::*;
    use ccbr_agents::models::{AgentSpec, WorkspaceMode};
