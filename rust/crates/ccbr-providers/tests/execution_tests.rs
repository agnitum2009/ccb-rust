...
fn test_execution_registry_case_insensitive() {
    ...
    registry.register(Box::new(
        ccbr_providers::providers::claude::ClaudeExecutionAdapter,
    ));
...
impl ccbr_providers::execution::ExecutionAdapter for FakeEmittingAdapter {
    ...
    fn poll(&self, submission: &ProviderSubmission, now: &str) -> Option<ProviderPollResult> {
        let item = CompletionItem::new(
            ...
            now.to_string(),
            ccbr_completion::models::CompletionCursor::new(
                CompletionSourceKind::ProtocolEventStream,
                now,
            ),
