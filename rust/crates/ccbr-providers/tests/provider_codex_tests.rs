...
fn test_codex_manifest_and_backend_registry() {
    ...
    assert!(backend
        .manifest
        .supports_runtime_mode(&ccbr_provider_core::manifest::RuntimeMode::PaneBacked));
...
fn test_codex_execution_adapter_poll_full_turn() {
    ...
    let request_anchor = ccbr_provider_core::protocol::request_anchor_for_job("j1");
    let req_id = ccbr_provider_core::protocol::make_req_id("j1");

    let mut file = std::fs::File::create(&log_path).unwrap();
    writeln!(
        file,
        r#"{{"type":"event_msg","payload":{{"type":"user_message","message":"{prefix} {anchor}"}},"timestamp":"t1"}}"#,
        prefix = ccbr_provider_core::protocol::REQ_ID_PREFIX,
...
fn test_codex_execution_adapter_poll_turn_aborted() {
    let tmp = tempfile::TempDir::new().unwrap();
    let log_path = tmp.path().join("codex-session.jsonl");
    let request_anchor = ccbr_provider_core::protocol::request_anchor_for_job("j2");
...
fn test_codex_log_entry_extraction_variants() {
    use ccbr_providers::providers::codex::CodexExecutionAdapter;
...
fn test_codex_anchor_fallback_log_scanning() {
    ...
    let request_anchor = ccbr_provider_core::protocol::request_anchor_for_job("j-fallback");
...
fn test_codex_session_refresh_picks_new_log() {
    ...
    let request_anchor = ccbr_provider_core::protocol::request_anchor_for_job("j-refresh");
    let req_id = ccbr_provider_core::protocol::make_req_id("j-refresh");
...
fn test_codex_terminal_decision_includes_diagnostics() {
    ...
    std::fs::create_dir(&workspace).unwrap();
    let log_path = workspace.join("codex-session.jsonl");
    let request_anchor = ccbr_provider_core::protocol::request_anchor_for_job("j-diag");
    let mut file = std::fs::File::create(&log_path).unwrap();
    writeln!(
        file,
        r#"{{"type":"event_msg","payload":{{"type":"user_message","message":"{prefix} {anchor}"}},"timestamp":"t1"}}"#,
