...
const SESSION_FILENAME: &str = ".copilot-session";
const SESSION_ID_ATTR: &str = "copilot_session_id";
const SESSION_PATH_ATTR: &str = "copilot_session_path";

// ---------------------------------------------------------------------------
// Manifest / backend
// ---------------------------------------------------------------------------

/// Build the Copilot provider manifest.
pub fn manifest() -> ProviderManifest {
    pane_backed_manifest(PROVIDER_NAME, false)
}

/// Build the Copilot provider backend registration.
...
/// Build a generic native CLI execution adapter configured for Copilot.
pub fn build_execution_adapter() -> NativeCliExecutionAdapter {
    NativeCliExecutionAdapter::new(
        NativeCliExecutionConfig::new(PROVIDER_NAME, _build_command)
            ...
            .with_reason("start_failed", "copilot_run_start_failed")
            .with_reason("failed", "copilot_run_failed")
            .with_reason("empty", "copilot_empty_reply")
            .with_reason("run_error", "copilot_run_error")
            .with_reason("complete", "copilot_run_stop")
            .with_reason("process_exit_complete", "copilot_run_exit")
            .with_reason("timeout", "copilot_run_timeout"),
    )
}
...
fn _build_env(request: &NativeCliExecutionRequest) -> HashMap<String, String> {
    let copilot_home = request.state_path("copilot_home", "home");
    let _ = std::fs::create_dir_all(&copilot_home);
    let mut env = HashMap::new();
    env.insert(
        "COPILOT_HOME".to_string(),
        copilot_home.to_string_lossy().to_string(),
    );
    env
}
...
/// Copilot execution adapter.
pub struct CopilotExecutionAdapter;

impl ExecutionAdapter for CopilotExecutionAdapter {
    ...
    fn start(
        ...
        let prompt_text = wrap_copilot_prompt(&job.request.body, &req_id);
...
/// A request to the Copilot provider.
...
/// The result of a Copilot request.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Wrap a prompt with Copilot request/done marker instructions.
///
/// Mirrors Python `provider_backends.copilot.protocol_runtime.wrap_copilot_prompt`,
/// adapted to the Rust `ccbr-provider-core` marker format (`<<DONE:req-id>>`).
pub fn wrap_copilot_prompt(message: &str, req_id: &str) -> String {
...
/// A loaded Copilot project session.
#[derive(Debug, Clone, Default)]
pub struct CopilotProjectSession {
    pub session_file: PathBuf,
    pub data: HashMap<String, Value>,
}

impl CopilotProjectSession {
    pub fn copilot_session_id(&self) -> String {
        self.data
            .get(SESSION_ID_ATTR)
            .and_then(|v| v.as_str())
            .unwrap_or("")
    ...
    pub fn copilot_session_path(&self) -> String {
        self.data
...
/// Load a Copilot project session.
pub fn load_project_session(
    work_dir: &Path,
    instance: Option<&str>,
) -> Option<CopilotProjectSession> {
    ...
    Some(CopilotProjectSession { session_file, data })
}
...
mod tests {
    ...
    fn test_extract_reply_for_req() {
        ...
        assert_eq!(extract_reply_for_req(text, "req-12345678"), "hello copilot");
    }
