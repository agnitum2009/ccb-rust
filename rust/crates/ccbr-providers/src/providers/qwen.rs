...
const SESSION_FILENAME: &str = ".qwen-session";
const SESSION_ID_ATTR: &str = "qwen_session_id";
const SESSION_PATH_ATTR: &str = "qwen_session_path";

// ---------------------------------------------------------------------------
// Manifest / backend
// ---------------------------------------------------------------------------

/// Build the Qwen provider manifest.
pub fn manifest() -> ProviderManifest {
    pane_backed_manifest(PROVIDER_NAME, false)
}

/// Build the Qwen provider backend registration.
...
pub struct QwenExecutionAdapter;

impl ExecutionAdapter for QwenExecutionAdapter {
    ...
    fn start(
        ...
        let prompt_text = wrap_qwen_prompt(&job.request.body, &req_id);
...
/// A request to the Qwen provider.
...
/// Wrap a prompt with Qwen request/done marker instructions.
///
/// Mirrors Python `provider_backends.qwen.protocol_runtime.wrap_qwen_prompt`,
/// adapted to the Rust `ccbr-provider-core` marker format (`<<DONE:req-id>>`).
pub fn wrap_qwen_prompt(message: &str, req_id: &str) -> String {
...
/// A loaded Qwen project session.
#[derive(Debug, Clone, Default)]
pub struct QwenProjectSession {
    pub session_file: PathBuf,
    pub data: HashMap<String, Value>,
}

impl QwenProjectSession {
    pub fn qwen_session_id(&self) -> String {
        self.data
            .get(SESSION_ID_ATTR)
            .and_then(|v| v.as_str())
            .unwrap_or("")
    ...
    pub fn qwen_session_path(&self) -> String {
        self.data
...
/// Load a Qwen project session.
pub fn load_project_session(work_dir: &Path, instance: Option<&str>) -> Option<QwenProjectSession> {
    ...
    Some(QwenProjectSession { session_file, data })
}
...
mod tests {
    ...
    fn test_execution_adapter_provider_name() {
        let adapter = QwenExecutionAdapter;
    ...
    fn test_poll_emits_terminal_decision_on_done_marker() {
        ...
        assert_eq!(decision.reply, "hello qwen");
    }

    #[test]
    fn test_wrap_qwen_prompt_format() {
        let wrapped = wrap_qwen_prompt("do the thing", "req-12345678");
    ...
    fn test_extract_reply_for_req() {
        ...
        assert_eq!(extract_reply_for_req(text, "req-12345678"), "hello qwen");
    }
    ...
    fn test_load_project_session() {
        ...
        std::fs::write(&session_path, r#"{"qwen_session_id":"s1"}"#).unwrap();

        let session = load_project_session(tmp.path(), None).unwrap();
        assert_eq!(session.qwen_session_id(), "s1");
