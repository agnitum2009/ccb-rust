...
pub const SESSION_FILENAME: &str = ".kimi-session";

/// Build the Kimi session binding.
pub fn build_session_binding() -> ProviderSessionBinding {
    ProviderSessionBinding {
        provider: PROVIDER_NAME.to_string(),
        session_id_attr: "kimi_session_id".to_string(),
        session_path_attr: "kimi_session_path".to_string(),
    }
}

/// A Kimi project session loaded from disk.
#[derive(Debug, Clone)]
...
impl KimiProjectSession {
    pub fn kimi_session_id(&self) -> String {
        self.data
            .get("kimi_session_id")
            .or_else(|| self.data.get("ccbr_session_id"))
            .and_then(|v| v.as_str())
            .unwrap_or("")
            .trim()
            .to_string()
...
/// Find the Kimi session file for a work directory.
...
/// Load the Kimi project session for a work directory.
///
/// Mirrors Python `provider_backends.kimi.session.load_project_session`: try the
/// instance-specific filename first, then fall back to the default filename.
...
fn try_load_project_session(work_dir: &Path, instance: Option<&str>) -> Option<KimiProjectSession> {
...
mod tests {
    ...
    fn test_load_project_session() {
        ...
        assert_eq!(session.kimi_session_id(), "session-1");
