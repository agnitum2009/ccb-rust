...
pub fn materialize_claude_home_config(
    ...
    let source_root: Utf8PathBuf = source_home.map(expand_user_path).unwrap_or_else(|| {
        Utf8PathBuf::from_path_buf(ccbr_provider_core::source_home::current_provider_source_home())
...
fn materialize_claude_settings(
    ...
    if !inherits_api(profile) {
        ...
        if let Some(env) = payload.get_mut("env").and_then(|v| v.as_object_mut()) {
            for key in ccbr_provider_profiles::provider_api_env_keys("claude") {
                env.remove(&key);
            }
        }
    }
    ...
    ccbr_storage::atomic::atomic_write_json(
...
fn materialize_claude_memory(
    ...
    agent_name: Option<&str>,
    workspace_path: Option<&Utf8Path>,
) -> anyhow::Result<ccbr_provider_core::memory_projection::MemoryProjectionResult> {
    ...
    ccbr_storage::atomic::atomic_write_text(&target, &rendered)?;
    let sha = ccbr_provider_core::memory_projection::text_file_sha256(target.as_std_path());
...
fn ensure_trust_file(path: &Utf8Path) -> anyhow::Result<()> {
    ...
    ccbr_storage::atomic::atomic_write_json(path, &serde_json::json!({}))?;
    Ok(())
}
...
fn write_json_object(
    ...
    ccbr_storage::atomic::atomic_write_json(path, &serde_json::Value::Object(obj.clone()))?;
    Ok(())
}
