//! Shared provider workspace preparation.
//!
//! Mirrors Python `lib/cli/services/provider_hooks.py` and is consumed by both
//! `ccbr-cli` and `ccbr-daemon` so that daemon start-up can materialize provider
//! homes and install hooks without depending on the CLI crate.
...
use ccbr_provider_hooks::settings::{
...
use ccbr_provider_profiles::materializer::{
...
use crate::claude::launcher_runtime::binary_cache::route_claude_binary_cache;
use crate::claude::launcher_runtime::home::{
    materialize_claude_home_config, resolve_claude_home_layout,
};
use crate::droid::paths::materialize_droid_home_config;
use crate::opencode::launcher::materialize_opencode_memory_config;
use crate::providers::gemini::materialize_gemini_home_config;
...
pub fn prepare_workspace_provider_hooks(
    ...
    let finish_hook = provider_hook_binary_path("ccbr-provider-finish-hook")?;
    ...
    if normalized == "claude" {
        if let (Some(project_id), Some(runtime_dir)) = (project_id, runtime_dir) {
            let activity_hook = provider_hook_binary_path("ccbr-provider-activity-hook")?;
...
fn _record_claude_binary_cache_drift_if_present(
    ...
    ccbr_storage::atomic::atomic_write_json(&marker_path, &signature)?;
