...
where
    ...
    B: FnOnce(&CliContext) -> Result<ccb_workspace::reconcile::WorkspaceGuardSummary, String>,
    H: FnOnce(StartSummary, ccb_workspace::reconcile::WorkspaceGuardSummary) -> StartSummary,
    W: FnOnce(&CliContext) -> Option<Value>,
{
    let guard_summary = before_client_start_fn(context)?;
    if !guard_summary.blockers.is_empty() {
        return Err(ccb_workspace::reconcile::format_workspace_blockers(
            "ccb start",
