...
//!
//! Thin wrappers around `ccb_runtime_pid_cleanup` so the CLI kill service
//! receives Python-compatible signatures and can pass `AgentRuntime` values
//! directly.

use ccb_agents::models::AgentRuntime;
use ccb_runtime_pid_cleanup::RuntimeRef;
use std::collections::HashMap;
use std::path::{Path, PathBuf};

pub use ccb_runtime_pid_cleanup::{
...
pub fn collect_agent_pid_candidates(
    ...
    ccb_runtime_pid_cleanup::collect_pid_candidates(agent_dir, dyn_ref, fallback_to_agent_dir)
}
...
pub fn collect_project_process_candidates(project_root: &Path) -> HashMap<u32, Vec<PathBuf>> {
    ccb_runtime_pid_cleanup::collect_project_process_candidates(
        project_root,
        Path::new("/proc"),
        ccb_runtime_pid_cleanup::read_proc_cmdline,
        Some(std::process::id()),
    )
}
