...
pub fn get_version_info(dir_path: &Path) -> Map<String, Value> {
    ...
    merge(
        &mut info,
        read_embedded_version_info(&dir_path.join("ccb")),
    );
...
///
/// Mirrors Python `read_embedded_version_info(ccb_file)`.
pub fn read_embedded_version_info(ccb_file: &Path) -> Map<String, Value> {
    let content = match fs::read_to_string(ccb_file) {
        Ok(c) => c,
