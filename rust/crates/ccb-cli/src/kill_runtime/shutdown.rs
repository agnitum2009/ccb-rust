...
fn finalize_shutdown_lifecycle(paths: &ccb_storage::paths::PathLayout, _socket_path: &Path) {
    let store = JsonStore::new();
...
fn inspect_daemon(paths: &ccb_storage::paths::PathLayout) -> (Value, Value, Value) {
    let store = JsonStore::new();
...
mod tests {
    ...
    fn make_paths(tmp: &tempfile::TempDir) -> ccb_storage::paths::PathLayout {
    ...
    fn make_context(paths: &ccb_storage::paths::PathLayout) -> CliContext {
