...
use ccb_cli::services::socket_path_for_project;
...
fn test_cli_maintenance_tick_healthy_writes_status() {
    ...
    let layout = ccb_storage::paths::PathLayout::new(project);
    let project_id = layout.project_id().to_string();
    let store = ccb_heartbeat::MaintenanceHeartbeatStore::new(layout, &project_id).unwrap();
    assert_eq!(
        store.load_status().state,
        ccb_heartbeat::store::ReadState::Ok
    );
...
/// used by `ccb_cli::services::UnixDaemonClient` (`{"method":..., "params":...}`).
...
fn test_cli_maintenance_tick_concern_submits_activation() {
    ...
    assert_eq!(status.state, ccb_heartbeat::store::ReadState::Ok);
