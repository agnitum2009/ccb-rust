...
pub fn collect_project_process_candidates(
    ...
    let ccbr_root = layout.ccbr_dir();
    let markers = project_runtime_markers(project_root, &ccbr_root, &layout);
    if markers.is_empty() {
        return HashMap::new();
    }
...
fn load_helper_manifest_best_effort(path: &Path) -> Option<u32> {
    ...
    let manifest = ccbr_providers::runtime::load_helper_manifest(utf8_path)?;
...
fn project_runtime_markers(
    ...
    let mut markers: Vec<PathBuf> = vec![ccbr_root.into()];
    if matches!(
        layout.runtime_state_placement().root_kind,
        ccbr_storage::path_helpers::RootKind::Relocated
    ) {
        for path in [
...
fn build_layout(project_root: &Path) -> Option<ccbr_storage::paths::PathLayout> {
    let utf8_path = Utf8Path::from_path(project_root)?;
    Some(ccbr_storage::paths::PathLayout::new(utf8_path))
}
