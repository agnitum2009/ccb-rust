...
use ccb_providers::providers::codebuddy::{
    backend, extract_reply_for_req, find_project_session_file, is_done_text, load_project_session,
    manifest, wrap_codebuddy_prompt, CodeBuddyExecutionAdapter, CodeBuddyRequest, CodeBuddyResult,
    CodebuddyProjectSession, PROVIDER_NAME,
};
...
fn test_backend_includes_binding_and_launcher() {
    ...
    assert_eq!(binding.session_id_attr, "codebuddy_session_id");
    assert_eq!(binding.session_path_attr, "codebuddy_session_path");
...
#[test]
fn test_wrap_codebuddy_prompt_strips_trailing_whitespace() {
    let wrapped = wrap_codebuddy_prompt("  test  \n\n", "req-12345678");
...
fn test_find_project_session_file() {
    ...
    let session_path = tmp.path().join(".codebuddy-session");
...
fn test_project_session_default() {
    let session = CodebuddyProjectSession::default();
    assert!(session.codebuddy_session_id().is_empty());
    assert!(session.codebuddy_session_path().is_empty());
}
