...
use ccb_providers::providers::qwen::{
    backend, extract_reply_for_req, find_project_session_file, is_done_text, load_project_session,
    manifest, wrap_qwen_prompt, QwenExecutionAdapter, QwenProjectSession, QwenRequest, QwenResult,
    PROVIDER_NAME,
};
...
fn test_backend_includes_binding_and_launcher() {
    ...
    assert_eq!(binding.session_id_attr, "qwen_session_id");
    assert_eq!(binding.session_path_attr, "qwen_session_path");
...
fn test_poll_emits_terminal_decision_on_done_marker() {
    ...
    let reply = format!(
        "{}\nhello qwen\n{}",
        ccb_provider_core::protocol::request_anchor_for_job(&job.job_id),
...
#[test]
fn test_wrap_qwen_prompt_strips_trailing_whitespace() {
    let wrapped = wrap_qwen_prompt("  test  \n\n", "req-12345678");
...
fn test_extract_reply_for_req_empty_on_wrong_id() {
    let text = "content\n<<DONE:req-87654321>>";
    let reply = extract_reply_for_req(text, "req-12345678");
    // A done marker for a different request id should yield an empty reply.
    assert_eq!(reply, "");
}
...
fn test_find_project_session_file() {
    ...
    let session_path = tmp.path().join(".qwen-session");
...
fn test_project_session_default() {
    let session = QwenProjectSession::default();
    assert!(session.qwen_session_id().is_empty());
    assert!(session.qwen_session_path().is_empty());
}
