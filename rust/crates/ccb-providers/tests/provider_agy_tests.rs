...
use ccb_providers::execution::target::{with_prompt_target_override, PromptTarget};
use ccb_providers::execution::{ExecutionAdapter, ProviderRuntimeContext};
use ccb_providers::providers::agy::{
...
fn start_with_mock_target(
    ...
    now: &str,
) -> ccb_providers::execution::ProviderSubmission {
...
fn test_resume_returns_none() {
    ...
    let submission = adapter.start(&job, None, &fake_now());
    let persisted = ccb_providers::execution::PersistedExecutionState::new(
        submission.clone(),
        None,
        false,
        fake_now(),
