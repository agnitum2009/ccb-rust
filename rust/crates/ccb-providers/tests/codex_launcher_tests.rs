...
fn test_codex_launcher_build_start_cmd_uses_materialized_profile_home() {
    ...
    let profile = ccb_provider_profiles::models::ResolvedProviderProfile {
        provider: "codex".to_string(),
...
fn test_codex_launcher_build_start_cmd_uses_agent_scoped_resume_session() {
    ...
    prepare_codex_home_overrides_for_test(
        ...
        Some(
            &camino::Utf8PathBuf::from_path_buf(
                ccb_dir.join("agents").join("agent1").join("events.jsonl"),
            )
            .unwrap(),
        ),
    ...
    std::fs::write(
        ccb_dir.join(".codex-agent1-session"),
        serde_json::json!({
    ...
    .unwrap();
    std::fs::write(
        ccb_dir.join(".codex-agent2-session"),
        serde_json::json!({
