...
use ccb_providers::providers::mimo::{backend, manifest, MimoExecutionAdapter, PROVIDER_NAME};
...
fn write_session_file_for_instance(work_dir: &Path, instance: Option<&str>) {
    ...
    write_json(
        ...
        &filename,
        serde_json::json!({
            "ccb_session_id": "session-1",
            "mimo_session_id": "session-1",
            "ccb_project_id": "proj1",
            "workspace_path": work_dir.to_string_lossy().to_string(),
            "work_dir": work_dir.to_string_lossy().to_string(),
            "pane_id": "%1",
            "runtime_dir": work_dir.join(".ccb").join("runtime").join("mimo").to_string_lossy().to_string(),
            "completion_artifact_dir": work_dir.join(".ccb").join("runtime").join("mimo").join("completion").to_string_lossy().to_string(),
