...
use ccb_completion::models::{
    CompletionConfidence, CompletionCursor, CompletionDecision, CompletionSourceKind,
...
use crate::execution::{
    error_submission, ExecutionAdapter, ProviderPollResult, ProviderRuntimeContext,
    ProviderSubmission,
};
...
fn start_active_submission(
    ...
    runtime_state.insert("prompt_sent".to_string(), Value::Bool(true));
    runtime_state.insert("send_error".to_string(), Value::Null);
...
fn poll_submission(submission: &ProviderSubmission, now: &str) -> Option<ProviderPollResult> {
    ...
    // No terminal condition and no meaningful progress to report.
    None
}
