...
use ccb_provider_core::contracts::{LaunchMode, ProviderBackend, ProviderRuntimeLauncher};
use ccb_provider_core::manifest::{CompletionManifest, ProviderManifest, RuntimeMode};
use ccb_provider_core::protocol::request_anchor_for_job;
        ...
        if reply_matches_request(&request_anchor, observed_req_id, no_wrap) {
            ...
            if !cleaned.is_empty() {
                ...
                decision = Some(CompletionDecision {
                    ...
                    confidence: Some(ccb_completion::models::CompletionConfidence::Observed),
                    reply: cleaned.clone(),
                    anchor_seen: true,
                    reply_started: true,
                    reply_stable: true,
