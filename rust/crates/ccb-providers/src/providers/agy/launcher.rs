...
pub fn build_start_cmd(
    ...
    if command.restore && !has_restore_arg(&cmd_parts) && !has_restore_arg(&spec.startup_args) {
...
fn resolve_agy_home(runtime_dir: &Utf8Path) -> PathBuf {
    crate::session_paths::state_dir_for_runtime_dir(runtime_dir)
        .map(|p| p.join("home"))
        .unwrap_or_else(|| runtime_dir.as_std_path().join("agy-home"))
}
