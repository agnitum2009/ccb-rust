...
use ccb_agents::policy::{resolve_effective_restore_mode, should_restore_provider_history};
use ccb_provider_core::caller_env::{caller_context_env, provider_user_session_env};
use ccb_provider_core::runtime_shared::{apply_provider_command_template, provider_start_parts};
use ccb_provider_profiles::codex_home_config::codex_provider_authority_fingerprint;
use ccb_provider_profiles::models::{ProviderProfileSpec, ResolvedProviderProfile};
use ccb_provider_profiles::provider_api_env_keys;
...
fn codex_args(
    ...
    let requested_restore = if command.restore {
        Some(ccb_agents::models::RestoreMode::Provider)
    } else {
        None
    };
