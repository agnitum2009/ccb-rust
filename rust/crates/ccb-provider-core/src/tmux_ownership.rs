...
pub fn session_user_option_lookup(session: &Session) -> HashMap<String, String> {
    ...
    if let Some(project_id) = session_data_text(session, "ccb_project_id") {
        lookup.insert("@ccb_project_id".to_string(), project_id);
    }
    if let Some(session_id) = session_data_text(session, "ccb_session_id") {
...
pub fn session_slot_user_option_lookup(session: &Session) -> HashMap<String, String> {
    ...
    if let Some(slot_key) = session_data_text(session, "ccb_slot") {
        lookup.insert("@ccb_slot".to_string(), slot_key);
    }
    if let Some(managed_by) = session_data_text(session, "ccb_managed_by") {
