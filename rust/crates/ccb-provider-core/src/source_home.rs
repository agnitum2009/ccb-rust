...
pub fn current_provider_source_home() -> PathBuf {
    ...
    if let Some(path) = env_path("HOME") {
        if !looks_like_ccb_provider_home(&path) {
            return path;
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_looks_like_ccb_provider_home() {
        assert!(looks_like_ccb_provider_home(&PathBuf::from(
            "/x/agents/foo/provider-state/bar/home"
        )));
        assert!(!looks_like_ccb_provider_home(&PathBuf::from("/home/user")));
        assert!(!looks_like_ccb_provider_home(&PathBuf::from(
