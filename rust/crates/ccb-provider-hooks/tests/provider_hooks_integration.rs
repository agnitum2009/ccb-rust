use camino::Utf8Path;
use ccb_provider_hooks::{activity, artifacts, notifications, settings};
use serde_json::json;
...
fn test_crate_root_reexports_reachable() {
    ...
    let _: Option<ccb_provider_hooks::ProviderActivityEvidence> = None;

    let _ = ccb_provider_hooks::COMPLETION_STATUS_CANCELLED;
    let _ = ccb_provider_hooks::COMPLETION_STATUS_COMPLETED;
    let _ = ccb_provider_hooks::COMPLETION_STATUS_FAILED;
    let _ = ccb_provider_hooks::COMPLETION_STATUS_INCOMPLETE;
    ...
    // activity
    let _ = ccb_provider_hooks::activity_path(&runtime);
    let _ = ccb_provider_hooks::load_activity(&runtime);
    let _ = ccb_provider_hooks::normalize_activity_state("tool");
    let _ = ccb_provider_hooks::read_activity_evidence(
        &runtime, "p", "a", "claude", None, None, None, None, None, 30.0,
    );
    let _ = ccb_provider_hooks::write_activity(
        "claude",
        "p",
        "a",
        &runtime,
    ...
    // artifacts
    let _ = ccb_provider_hooks::completion_dir_from_session_data(&HashMap::new());
    let _ = ccb_provider_hooks::event_path(&completion, "id");
    let _ = ccb_provider_hooks::extract_req_id("CCB_REQ_ID: job_1");
    let _ = ccb_provider_hooks::latest_req_id_from_transcript(None);
    let _ = ccb_provider_hooks::load_event(&completion, "id");
    ...
    // notifications
    let _ = ccb_provider_hooks::completion_status_label(Some("failed"), true);
    let _ = ccb_provider_hooks::completion_status_marker(Some("failed"), true);
    let _ = ccb_provider_hooks::default_reply_for_status(Some("failed"), true);
    let _ = ccb_provider_hooks::normalize_completion_status(Some("failed"), true);

    // settings
    let _ = ccb_provider_hooks::build_hook_command(
        "claude",
    ...
    let _ = ccb_provider_hooks::build_activity_hook_command(
        "claude",
        Utf8Path::new("/tmp/script"),
        "/usr/bin/python3",
        "p",
    ...
    let _ = ccb_provider_hooks::install_workspace_activity_hooks(
        "claude",
        &workspace,
        Some(&home_root),
        "cmd",
    );
    let _ = ccb_provider_hooks::install_workspace_completion_hooks(
        "claude",
        &workspace,
        Some(&home_root),
        "cmd",
    ...
    // submodule helpers
    let _ = ccb_provider_hooks::claude_hook_home_layout(&home_root);
    let _ = ccb_provider_hooks::load_json(root.join("missing.json").as_ref());
    let _ = ccb_provider_hooks::save_json(root.join("test.json").as_ref(), &Map::new());
    let _ = ccb_provider_hooks::workspace_key(&workspace);
}
