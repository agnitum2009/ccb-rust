...
pub fn history_dir(work_dir: &Path) -> PathBuf {
    ...
    ccb_provider_sessions::files::resolve_project_config_dir(&resolved).join("history")
}
...
fn run_auto_transfer(
    ...
    let last_n = ccb_types::env::env_int("CCB_CTX_TRANSFER_LAST_N", 3) as usize;
    let max_tokens = ccb_types::env::env_int("CCB_CTX_TRANSFER_MAX_TOKENS", 8000) as u32;
