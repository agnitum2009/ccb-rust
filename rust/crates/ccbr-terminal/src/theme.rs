...
pub fn pane_border_format(
    ...
    format!(
        "#{{?#{{@ccbr_agent}},#{{?#{{@ccbr_label_style}},#{{@ccbr_label_style}},{fallback}}} #{{@ccbr_agent}} #[default],#[fg=#565f89] #{{pane_title}} #[default]}}",
...
pub fn render_tmux_session_theme(
    ...
    let profile = theme_profile_definition(profile_name, environ);
    let normalized_version = normalized_ccbr_version(ccbr_version);
    let focus_agent = "#{?#{@ccbr_agent},#{@ccbr_agent},-}";
...
fn normalized_ccbr_version(value: &str) -> String {
    let s = value.trim();
    if s.is_empty() {
        "?".to_string()
    } else {
