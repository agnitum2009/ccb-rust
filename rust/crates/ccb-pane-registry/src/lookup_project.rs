...
mod tests {
    ...
    impl TerminalBackend for MockBackend {
        fn send_text(&self, _pane_id: &str, _text: &str) -> ccb_terminal::backend::Result<()> {
            Ok(())
        }
        fn is_alive(&self, pane_id: &str) -> ccb_terminal::backend::Result<bool> {
            Ok(self.alive.get(pane_id).copied().unwrap_or(false))
        }
        fn kill_pane(&self, _pane_id: &str) -> ccb_terminal::backend::Result<()> {
            Ok(())
        }
        fn activate(&self, _pane_id: &str) -> ccb_terminal::backend::Result<()> {
            Ok(())
        }
        fn create_pane(
            &self,
        ...
        ) -> ccb_terminal::backend::Result<String> {
            Ok("%0".into())
        }
    }
    ...
    fn test_existing_project_id_from_record() {
        let mut data = Map::new();
        data.insert("ccb_project_id".into(), "  pid  ".into());
    ...
    fn test_migrate_project_id() {
        ...
        migrate_project_id(&mut data, &compute_id_fn, &|_record: &Map<
            ...
            Value,
        >| true);
        assert_eq!(data["ccb_project_id"], "id-for-/tmp/proj");
    }

    #[test]
    fn test_migrate_project_id_skips_existing() {
        let mut data = sample_record();
        data.insert("ccb_project_id".into(), "existing".into());
        migrate_project_id(&mut data, &compute_id_fn, &|_record: &Map<
            String,
            Value,
        >| true);
        assert_eq!(data["ccb_project_id"], "existing");
    }
}
