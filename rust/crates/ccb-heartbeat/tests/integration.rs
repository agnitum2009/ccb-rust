use ccb_heartbeat::engine::evaluate_heartbeat;
use ccb_heartbeat::lock::MaintenanceHeartbeatLock;
use ccb_heartbeat::maintenance::{evaluate_project_view, evaluate_ps_summary};
...
use ccb_heartbeat::store::{HeartbeatStateStore, MaintenanceHeartbeatStore, ReadState};
...
#[test]
fn maintenance_classifier_keeps_active_ccb_job_healthy() {
    let mut agent = base_agent();
    agent["activity_state"] = "active".into();
    agent["activity_reason"] = "job_running".into();
    agent["activity_source"] = "ccb_job".into();
