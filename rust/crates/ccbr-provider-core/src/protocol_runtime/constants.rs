...
pub const DONE_LINE_RE_TEMPLATE: &str = r"^\s*CCBR_DONE:\s*{req_id}\s*$";

/// Regex pattern (as a string) matching any done line.
pub const ANY_DONE_LINE_RE: &str =
    r"(?i)^\s*CCBR_DONE:\s*(?:job_[a-z0-9]+|[0-9a-fA-F]{32}|\d{8}-\d{6}-\d{3}-\d+-\d+)\s*$";
...
pub fn is_trailing_noise_line(line: &str) -> bool {
    ...
    // Match lines like `CLAUDE_DONE:` or `CLAUDE_DONE: <req_id>` but not `CCBR_DONE:`.
    if trimmed.to_ascii_uppercase().starts_with("CCBR_DONE:") {
        return false;
    }
...
mod tests {
    ...
    fn test_constants_match_expected_prefixes() {
        assert_eq!(BEGIN_PREFIX, "CCBR_BEGIN:");
        assert_eq!(DONE_PREFIX, "CCBR_DONE:");
        assert_eq!(REQ_ID_PREFIX, "CCBR_REQ_ID:");
    }
    ...
    fn test_any_done_line_re_matches_variants() {
        ...
        assert!(re.is_match("  ccbr_done:  job_abc123  "));
        assert!(re.is_match("CCBR_DONE: 0123456789abcdef0123456789abcdef"));
        assert!(re.is_match("CCBR_DONE: 20240102-030405-006-1234-1"));
        assert!(!re.is_match("CLAUDE_DONE: job_abc123"));
        assert!(!re.is_match("CCBR_DONE:"));
    }
    ...
    fn test_done_line_re_matches_specific_req_id() {
        ...
        assert!(re.is_match("CCBR_DONE: job_abc123"));
        assert!(re.is_match("  ccbr_done: job_abc123  "));
        assert!(!re.is_match("CCBR_DONE: job_def456"));
    }
    ...
    fn test_is_trailing_noise_line() {
        ...
        assert!(!is_trailing_noise_line("CCBR_DONE: job_abc123"));
