...
fn prune_ccbr_managed_hook_groups(
    ...
    for group in groups {
        ...
        let kept_hooks: Vec<serde_json::Value> = hooks
            .iter()
            .filter(|hook| !is_stale_ccbr_managed_hook(hook, current_command, managed_hook_name))
            .cloned()
            .collect();
...
mod tests {
    ...
    fn test_install_claude_hooks_prunes_stale_hooks() {
        ...
        let command = "/usr/bin/python3 /current/bin/ccbr-provider-finish-hook --provider claude";
        let stale_command = "/usr/bin/python3 /old/bin/ccbr-provider-finish-hook --provider claude";
        std::fs::write(
            &settings_path,
