...
use ccbr_cli::models_mailbox::ParsedAskCommand;
use ccbr_cli::services::ask::{message_with_reply_guidance, resolve_ask_sender, submit_ask_with};
use ccbr_cli::services::ask_runtime::submission::SubmitClient;
...
fn test_write_ask_output_appends_newline() {
    ...
    ccbr_cli::services::ask::write_ask_output(&path, "done").unwrap();
...
fn test_write_ask_output_preserves_existing_newline() {
    let tmp = tempfile::TempDir::new().unwrap();
    let path = tmp.path().join("reply.txt");
    ccbr_cli::services::ask::write_ask_output(&path, "done\n").unwrap();
    assert_eq!(std::fs::read_to_string(&path).unwrap(), "done\n");
}

#[test]
fn test_exit_code_for_ask_status() {
    use ccbr_cli::services::ask::exit_code_for_ask_status;
