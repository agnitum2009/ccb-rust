...
use ccbr_cli::ops_views_doctor::render_doctor;
use ccbr_cli::services::doctor_runtime::system::{runtime_identity_summary_with, user_name};
...
fn test_render_doctor_includes_root_runtime_identity_lines() {
    ...
    assert!(lines.contains(&"ccbr_dir_owner: 1000:demo".to_string()));
    assert!(lines.contains(
        &"runtime_warning: Running CCBR as root in a non-root-owned project can create root-owned .ccbr files.".to_string()
    ));
}
