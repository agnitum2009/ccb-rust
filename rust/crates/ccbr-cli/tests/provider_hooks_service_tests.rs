        ...
        entry
            ...
            .any(|h| {
                h.get("command")
                    .and_then(|c| c.as_str())
                    .unwrap_or("")
                    .contains("ccbr-provider-finish-hook")
            })
    }));
}
...
fn test_prepare_hooks_claude_installs_activity_hooks_with_project_and_runtime() {
    ...
    for event in events {
        ...
        assert!(
            has_command(arr, "ccbr-provider-activity-hook"),
    ...
    assert!(has_command(stop, "ccbr-provider-finish-hook"));
}
...
fn test_prepare_hooks_gemini_installs_completion_hooks() {
    ...
    assert!(!hooks
        ...
        .unwrap()
        .values()
        .any(|v| v.to_string().contains("ccbr-provider-activity-hook")));
}

#[test]
fn test_resolve_gemini_home_root() {
    let (_tmp, root) = tmp_dir();
    let layout = ccbr_storage::paths::PathLayout::new(root);
    let expected = layout
