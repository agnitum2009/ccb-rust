...
use ccbr_cli::kill_runtime::processes::{is_pid_alive, kill_pid_tree_once, proc_pid_state_at};
...
fn collect_project_process_candidates_finds_ccbd_project_arg() {
    ...
    let candidates = ccbr_runtime_pid_cleanup::collect_project_process_candidates(
        &project_root,
        &proc_root,
...
fn collect_project_authority_pid_candidates_reads_lifecycle() {
    ...
    let candidates =
        ccbr_runtime_pid_cleanup::collect_project_authority_pid_candidates(&project_root);
