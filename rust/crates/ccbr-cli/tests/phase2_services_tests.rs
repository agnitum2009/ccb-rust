...
use ccbr_cli::phase2_services::DaemonPhase2Services;
...
fn make_project() -> (TempDir, PathBuf) {
    ...
    std::fs::write(ccbr_dir.join("ccbr.config"), "agent1:codex\n").unwrap();
    (tmp, project_root)
}
...
fn test_phase2_doctor_bundle_uses_local_service() {
    ...
    let context = build_context(
        project_root.clone(),
        ParsedCommand::Doctor(ccbr_cli::models::ParsedDoctorCommand {
            project: None,
            bundle: true,
            output_path: None,
            storage: false,
    ...
    assert_eq!(code, 0, "exit code should be 0, output: {}", output);
    assert!(
        output.contains("bundle_path") || output.contains("ccbr-support"),
...
fn test_phase2_config_validate_uses_local_config_loader() {
    ...
    let context = build_context(
        project_root.clone(),
        ParsedCommand::ConfigValidate(ccbr_cli::models::ParsedConfigValidateCommand {
            project: None,
