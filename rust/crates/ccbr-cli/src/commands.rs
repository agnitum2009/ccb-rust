    ...
    let result = match cmd.action.as_str() {
        ...
        "runner" => {
            let runner_id = parse_runner_id(&cmd.args)
                .unwrap_or_else(|| format!("ccbr-runner-{}", std::process::id()));
...
pub fn config_validate(_cmd: &ParsedConfigValidate, project_root: &Path) -> Result<String, String> {
    ...
    let result = ccbr_agents::config::load_project_config(&layout)
...
pub fn roles(cmd: &ParsedRoles, project_root: &Path) -> Result<String, String> {
    match &cmd.action {
        RolesAction::List => {
            let installed_root = ccbr_agents::rolepacks::agent_roles_installed_root();
            let mut roles: Vec<ccbr_agents::rolepacks::RoleManifest> = Vec::new();
            if installed_root.is_dir() {
                if let Ok(entries) = std::fs::read_dir(&installed_root) {
                    for entry in entries.flatten() {
                        let path = entry.path();
                        if path.is_dir() {
                            if let Ok(manifest) = ccbr_agents::rolepacks::load_role_manifest(&path)
                            {
                                roles.push(manifest);
                            }
                        }
        ...
        RolesAction::Add { spec } => {
            ...
            let payload = ccbr_agents::rolepacks::add_role_to_project_config(
                project_root,
                &role_id,
                None,
                provider.as_deref(),
        ...
        RolesAction::Update { path } => {
            ...
            let payload = ccbr_agents::rolepacks::update_role(role_id, None, None, true)
                .map_err(|e| e.to_string())?;
            Ok(render_roles(&serde_json::Value::Object(payload)))
        }
        RolesAction::Install { path } => {
            ...
            let payload = ccbr_agents::rolepacks::install_role(role_id, None, None, true)
                .map_err(|e| e.to_string())?;
            Ok(render_roles(&serde_json::Value::Object(payload)))
        }
        RolesAction::Sync { path } => {
            let sync_path = path.as_deref().unwrap_or(".");
            let payload = ccbr_agents::rolepacks::sync_roles_from_path(
        ...
        RolesAction::Doctor { path } => {
            let payload =
                ccbr_agents::rolepacks::role_status(path, true).map_err(|e| e.to_string())?;
            Ok(render_roles(&serde_json::Value::Object(payload)))
        }
    }
}
...
pub fn autonew(cmd: &ParsedAutonew, project_root: &Path) -> Result<String, String> {
    ...
    let session_file =
        ccbr_provider_sessions::files::find_project_session_file(project_root, session_filename)
            .ok_or_else(|| {
    ...
    let backend = ccbr_terminal::backend::TmuxBackend::new(socket_name, socket_path);
...
pub fn ctx_transfer(cmd: &ParsedCtxTransfer, project_root: &Path) -> Result<String, String> {
    ...
    let transfer = ccbr_memory::ContextTransfer::new(cmd.max_tokens as u32, project_root);
