...
fn config_identity_payload(context: &CliContext) -> Value {
    match ccbr_agents::config::load_project_config(&context.paths) {
        Ok(result) => {
            let identity =
                ccbr_agents::config_identity::project_config_identity_payload(&result.config);
            json!({
...
mod tests {
    ...
    fn connect_compatible_daemon_probes_then_connects_with_runtime_timeout() {
        ...
        let config_result = ccbr_agents::config::load_project_config(&context.paths).unwrap();
        let identity =
            ccbr_agents::config_identity::project_config_identity_payload(&config_result.config);
