...
use ccbr_heartbeat::store::MaintenanceHeartbeatReadResult;
use ccbr_heartbeat::store::MaintenanceHeartbeatStore;
use ccbr_heartbeat::{
...
fn heartbeat_config(context: &CliContext) -> HeartbeatConfig {
    ...
    let result = match ccbr_agents::config::load_project_config(&context.paths) {
        Ok(r) => r,
...
fn assessor_present(context: &CliContext, assessor: &str) -> bool {
    ...
    ccbr_agents::config::load_project_config(&context.paths)
...
fn schedule_is_future(
    ...
    if let Some(value) = &schedule.value {
        if let Some(next_run_at) = &value.next_run_at {
            if let Some(secs) = ccbr_heartbeat::seconds_between(now, next_run_at) {
                return secs > 0.0;
            }
        }
    }
...
fn seconds_until(from: &str, until: Option<&str>) -> f64 {
    if let Some(until) = until {
        if let Some(secs) = ccbr_heartbeat::seconds_between(from, until) {
            return secs.max(0.0);
        }
    }
    0.0
...
fn build_activation(
    ...
    evidence: Vec<Value>,
) -> Result<MaintenanceHeartbeatActivation, ccbr_heartbeat::HeartbeatError> {
