...
use ccbr_workspace::reconcile::WorkspaceGuardSummary;
...
pub fn start_agents(
    ...
    let before_client_start_fn = |ctx: &CliContext| -> Result<WorkspaceGuardSummary, String> {
        let config_result = ccbr_agents::config::load_project_config(&ctx.paths)
            .map_err(|e| format!("failed to load project config: {e}"))?;
        ccbr_workspace::reconcile::reconcile_start_workspaces(
