...
pub const DEFAULT_CCB_SELF_AGENT: &str = "ccb_self";
pub const DEFAULT_CCB_SELF_ROLE: &str = "agentroles.ccb_self";
...
pub fn project_config_path(layout: &PathLayout) -> Utf8PathBuf {
    layout.ccb_dir().join(CONFIG_FILENAME)
}
...
pub fn build_default_project_config() -> ProjectConfig {
    ...
    agents.insert(
        "ccb_self".into(),
        build_default_agent_spec("ccb_self", "codex", Some(DEFAULT_CCB_SELF_ROLE)),
    );
...
fn provider_profile_config_dict(spec: &AgentSpec) -> Option<serde_json::Value> {
    ...
    let api_keys = ccb_provider_profiles::materializer::provider_api_env_keys(&spec.provider);
