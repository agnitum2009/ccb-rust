...
pub use ccbr_provider_profiles::ProviderProfileSpec;
use regex::Regex;
...
pub const DEFAULT_MAINTENANCE_HEARTBEAT_ASSESSOR: &str = "ccbr_self";
...
impl AgentSpec {
        ...
        if let Some(tpl) = &self.provider_command_template {
            ...
            if trimmed.matches(PROVIDER_COMMAND_PLACEHOLDER).count() != 1 {
                return Err(crate::AgentError::ProviderCore(
                    ccbr_provider_core::error::ProviderCoreError::InvalidCommandTemplate,
                ));
            }
    ...
    fn normalize_startup_args(&mut self) -> crate::Result<()> {
        if let Some(model) = &self.model {
            let compiled = ccbr_provider_core::model_shortcuts::provider_model_startup_args(
                &self.provider,
                model,
            )?;
            ...
            if normalized.starts_with(&compiled) {
                ...
                if ccbr_provider_core::model_shortcuts::startup_args_contain_model_flag(
                    &self.provider,
                    remaining,
                ) {
                    return Err(crate::AgentError::Validation(format!(
