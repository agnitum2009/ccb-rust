...
pub fn translate_agent_role_manifest(
    ...
    let adapter = load_ccbr_adapter(root)?;
...
fn load_ccbr_adapter(root: &Path) -> crate::Result<serde_json::Map<String, serde_json::Value>> {
...
fn _write_sources(sources: &[serde_json::Value]) -> crate::Result<()> {
    ...
    let utf8_path = camino::Utf8Path::from_path(&path).ok_or_else(|| {
        crate::AgentError::Storage(ccbr_storage::StorageError::Corrupt(
            "source registry path is not valid utf-8".into(),
        ))
    })?;
    ccbr_storage::atomic::atomic_write_json(utf8_path, &payload)?;
    Ok(())
}
...
pub fn resolve_project_agent_role(
    ...
    let layout = ccbr_storage::paths::PathLayout::new(utf8_root);
...
pub fn write_project_role_lock(project_root: &Path, role: &RoleManifest) -> crate::Result<()> {
    ...
    ccbr_storage::atomic::atomic_write_json(utf8_path, &serde_json::Value::Object(existing))?;
    Ok(())
}
...
pub fn add_role_to_project_config(
    ...
    let layout = ccbr_storage::paths::PathLayout::new(project_root.to_string_lossy().as_ref());
