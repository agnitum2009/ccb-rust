use ccbr_provider_sessions::files::{
    check_session_writable, find_project_session_file, project_config_dir, safe_write_session,
    ProviderClientSpec,
};
use ccbr_provider_sessions::resolution::{resolve_work_dir, resolve_work_dir_with_registry};
use ccbr_provider_sessions::watch::{is_watch_file, SessionFileWatcher};
use std::fs;
use std::path::Path;
...
fn test_crate_root_reexports_reachable() {
    ...
    let _: fn(&str) = ccbr_provider_sessions::print_session_error_stderr;

    let dir = TempDir::new().unwrap();
    let session = dir.path().join("session.json");

    let _ = ccbr_provider_sessions::check_session_writable(&session);
    let _ = ccbr_provider_sessions::find_project_session_file(dir.path(), ".claude-session");
    let _ = ccbr_provider_sessions::project_config_dir(dir.path());
    let _ = ccbr_provider_sessions::resolve_project_config_dir(dir.path());
    let _ = ccbr_provider_sessions::safe_write_session(&session, "hello");

    let spec = ccbr_provider_sessions::files::ProviderClientSpec::new("claude", ".claude-session");
    let _ = ccbr_provider_sessions::resolve_work_dir(&spec, None, None, None);
    let _ = ccbr_provider_sessions::resolve_work_dir_with_registry(
        &spec,
        "claude",
        None,
        None,
    ...
    let _watcher = ccbr_provider_sessions::SessionFileWatcher::with_predicate(
        dir.path().to_path_buf(),
        |_path: PathBuf| {},
        false,
        Some(|_path: &Path| true),
    );
    let _watcher = ccbr_provider_sessions::SessionFileWatcher::new_with_predicate(
        dir.path().to_path_buf(),
        |_path: PathBuf| {},
        false,
        Some(|_path: &Path| true),
