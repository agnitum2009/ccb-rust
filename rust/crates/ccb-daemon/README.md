...
Rust implementation of the CCB daemon control plane, migrated from `lib/ccbd/`
...
- **Completion tracking / message bureau integration**: dispatcher tracks jobs
  but does not yet integrate with `ccb-completion` or `ccb-mailbox` for actual
  provider execution.
...
- **Tmux layout integration**: start flow uses direct tmux session creation
  rather than the full `ccb-terminal` auto-layout backend.

## Testing

```bash
cd /home/agnitum/ccb/rust
cargo test -p ccb-daemon
cargo clippy -p ccb-daemon
```
