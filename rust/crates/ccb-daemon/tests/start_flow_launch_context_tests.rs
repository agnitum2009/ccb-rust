...
use ccb_daemon::provider_launcher::{LaunchContext, LaunchResult, Launcher};
use ccb_daemon::services::registry::{AgentRegistry, AgentRuntimeEntry};
...
fn test_start_flow_forwards_terminal_size_startup_timeout_and_flags() {
    ...
    let service = StartFlowService::with_launcher(
        ccb_daemon::start_flow::service::StartFlowMode::Stub,
        launcher.clone(),
    );
