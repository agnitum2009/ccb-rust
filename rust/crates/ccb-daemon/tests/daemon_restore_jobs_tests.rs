...
use ccb_daemon::models::api_models::records::JobRecord;
...
"#,
    ...
    app.completion_tracker = ccb_completion::CompletionTrackerService::new(
        app.current_config.clone().unwrap_or_default(),
        ccb_provider_core::catalog::build_default_provider_catalog(false, true),
        ccb_completion::CompletionRegistry,
    );
...
fn daemon_restart_restores_running_job_into_dispatcher_and_execution() {
    ...
    let completion_job = ccb_daemon::adapters::completion::to_completion_job_record(&job);
