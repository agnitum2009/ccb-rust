//! Integration tests for CCB daemon ownership persistence across restarts.

use camino::Utf8Path;
use ccb_daemon::CcbdApp;
use ccb_storage::paths::PathLayout;

fn make_test_app(project_root: &std::path::Path) -> CcbdApp {
    CcbdApp::with_backend(
        project_root,
        ccb_daemon::start_flow::service::StartFlowService::with_stub(),
        ccb_daemon::stop_flow::service::StopFlowService::with_stub(),
    )
}
...
fn ownership_save_load_roundtrip_preserves_record() {
    ...
    let state_path = layout.ccbd_dir().join("ownership-state.json");
