...
use ccb_agents::models::{AgentSpec, ProjectConfig, RestoreMode};
use ccb_agents::policy::resolve_agent_launch_policy;
use ccb_agents::store::{AgentRestoreStore, AgentRuntimeStore, AgentSpecStore};
use ccb_provider_profiles::materializer::validate_provider_runtime_home_uniqueness;
use ccb_providers::workspace_preparation::prepare_provider_workspace;
use ccb_storage::paths::PathLayout;
use ccb_workspace::binding::WorkspaceBindingStore;
use ccb_workspace::materializer::WorkspaceMaterializer;
use ccb_workspace::models::WorkspacePlan;
use ccb_workspace::planner::WorkspacePlanner;
use ccb_workspace::validator::WorkspaceValidator;
...
pub trait RestoreStateBuilder {
    fn build(&self, mode: &str) -> ccb_agents::models::AgentRestoreState;
}
...
fn effective_restore_mode_name(mode: ccb_agents::policy::EffectiveRestoreMode) -> &'static str {
    match mode {
        ccb_agents::policy::EffectiveRestoreMode::Fresh => "fresh",
        ccb_agents::policy::EffectiveRestoreMode::Provider => "provider",
        ccb_agents::policy::EffectiveRestoreMode::Attach => "attach",
        ccb_agents::policy::EffectiveRestoreMode::Memory => "memory",
        ccb_agents::policy::EffectiveRestoreMode::Auto => "auto",
    }
}
