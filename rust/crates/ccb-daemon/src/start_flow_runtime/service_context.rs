...
pub fn build_start_context(
    ...
    let context = StartFlowContext {
        ...
        project: StartFlowProjectContext {
            ...
            config_dir: paths.ccb_dir().as_std_path().to_path_buf(),
...
mod tests {
    ...
    fn test_build_start_context() {
        ...
        assert_eq!(context.cwd, PathBuf::from("/tmp/ccb-test"));
        assert_eq!(context.project.project_id, "ccb-test-id");
    ...
    fn test_record_namespace_action_records_when_configured() {
        ...
        assert_eq!(actions, vec!["ensure_namespace:epoch=7,session=ccb-sess"]);
    }

    #[test]
    fn test_record_namespace_action_skips_when_unconfigured() {
        let mut actions = Vec::new();
        record_namespace_action(&mut actions, None, Some("ccb-sess"), Some(7));
