...
type RunStartFlowFn<'a> = &'a dyn Fn(
    ...
    Option<Vec<ccb_agents::models::WindowSpec>>,
...
mod tests {
    ...
    fn sample_graph() -> ServiceGraph {
        let mut config = ccb_agents::models::ProjectConfig::default();
        config.agents.insert(
            "claude".to_string(),
            ccb_agents::models::AgentSpec::default_with_name("claude"),
        );
        ServiceGraph {
    ...
    fn test_run_additive_agent_mounts_with_start_flow() {
        ...
        let run_fn = |_root: &std::path::Path,
                      ...
                      _windows: Option<Vec<ccb_agents::models::WindowSpec>>| {
            Ok(StartFlowResult {
                status: "ok".to_string(),
