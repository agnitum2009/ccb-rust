...
/// Persisted lifecycle store for the CCB daemon.
///
/// Mirrors Python `ccbd.services.lifecycle.CcbdLifecycleStore`. Records are
/// saved to `.ccb/ccbd/lifecycle.json` and carry the daemon phase, startup
...
impl LifecycleStore {
    ...
    pub fn load(&self) -> Option<CcbdLifecycle> {
        let path = self.layout.ccbd_lifecycle_path();
        if !path.exists() {
            return None;
        }
    ...
    pub fn save(&self, lifecycle: &CcbdLifecycle) -> ccb_storage::Result<()> {
        let path = self.layout.ccbd_lifecycle_path();
    ...
    fn lifecycle_store_roundtrip_preserves_startup_progress_fields() {
        ...
        let lifecycle = build_lifecycle(
            ...
            CcbdLifecycleUpdates {
                ...
                socket_path: Some(Some(layout.ccbd_socket_path().to_string())),
                ..Default::default()
            },
        );
