...
//!
//! Wires the ccb-heartbeat engine into the daemon so running jobs that make
...
pub fn record_terminal_timeout(
    ...
    mailbox: &ccb_mailbox::bureau::MessageBureauFacade,
    job_id: &str,
    finished_at: &str,
) {
    ...
    let decision = ccb_mailbox::facade_recording::CompletionDecision {
        terminal: true,
        status: ccb_mailbox::models::JobStatus::Incomplete,
