...
//! Resolves the `ccb-agent-sidebar` helper binary and builds respawn arguments.

use std::collections::HashMap;
use std::path::{Path, PathBuf};

pub const SIDEBAR_BINARY_NAME: &str = "ccb-agent-sidebar";
...
/// 2. `<script_root>/bin/ccb-agent-sidebar`
/// 3. `$CODEX_INSTALL_PREFIX/bin/ccb-agent-sidebar`
/// 4. `PATH` lookup
...
///
/// If the first argument is `ccb-agent-sidebar`, it is replaced with the resolved
...
pub fn missing_sidebar_respawn_args(reason: Option<&str>) -> Vec<String> {
    ...
    let detail = reason.unwrap_or("ccb-agent-sidebar not found");
    let body = format!(
        "printf '%s\\n' '{}'; printf '%s\\n' '{}'; printf '%s\\n' 'Build or install bin/ccb-agent-sidebar, or set CCB_AGENT_SIDEBAR_BIN.'; while :; do sleep 3600; done",
...
fn default_script_root() -> PathBuf {
    ...
    // `rust/crates/ccb-daemon/src/services/project_namespace_runtime/`, so we
...
mod tests {
    ...
    fn test_resolve_explicit_missing() {
        let path = PathBuf::from("/non/existent/ccb-agent-sidebar");
        let resolution = resolve_explicit(&path, "TEST_SOURCE");
        assert!(!resolution.available());
