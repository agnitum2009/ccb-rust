...
use ccb_agents::models::{ProjectConfig, SidebarSpec, ToolWindowSpec, WindowSpec};
...
pub fn build_namespace_topology_plan(
    ...
    let sidebar_enabled = sidebar
        .map(|s| s.mode.trim() == ccb_agents::models::SIDEBAR_MODE_EVERY_WINDOW)
        .unwrap_or(false);
...
fn sidebar_launch_args(
    ...
    let mut args = vec!["ccb-agent-sidebar".to_string()];
