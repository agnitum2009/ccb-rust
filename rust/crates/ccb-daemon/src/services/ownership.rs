...
/// Persisted ownership record for the CCB daemon.
///
...
impl OwnershipService {
    ...
    pub fn load(&mut self) -> crate::Result<Option<OwnershipRecord>> {
        ...
        let record: OwnershipRecord = ccb_storage::json::JsonStore::new().load(path)?;
    ...
    pub fn save(&self) -> crate::Result<()> {
        ...
        if let Some(record) = self.ownership.as_ref() {
            ...
            ccb_storage::json::JsonStore::new().save(path, record)?;
        }
        Ok(())
    }
...
mod tests {
    ...
    fn test_save_load_roundtrip() {
        ...
        let record = service.acquire(1234, "/tmp/ccb.sock", "instance-a");
        service.save().unwrap();
    ...
    fn test_restore_or_acquire_reuses_same_holder() {
        ...
        let mut service = OwnershipService::with_state_path(path);
        let first = service.acquire(1234, "/tmp/ccb.sock", "instance-a");
        service.save().unwrap();

        let mut restarted = OwnershipService::with_state_path(path);
        let second = restarted
            .restore_or_acquire(1234, "/tmp/ccb.sock", "instance-a")
            .unwrap();
    ...
    fn test_restore_or_acquire_bumps_generation_for_new_holder() {
        ...
        let mut service = OwnershipService::with_state_path(path);
        service.acquire(1234, "/tmp/ccb.sock", "instance-a");
        service.save().unwrap();

        let mut restarted = OwnershipService::with_state_path(path);
        let second = restarted
            .restore_or_acquire(5678, "/tmp/ccb.sock", "instance-b")
            .unwrap();
