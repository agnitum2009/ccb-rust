...
use ccb_mailbox::facade_recording::CompletionDecision as MailboxCompletionDecision;
use ccb_mailbox::models::{
    AttemptRecord, AttemptState, CallbackEdgeRecord, CallbackEdgeState, MessageState,
    ReplyTerminalStatus,
};
use ccb_mailbox::stores::{AttemptStore, CallbackEdgeChanges};
use ccb_storage::text_artifacts::{maybe_spill_text, TEXT_ARTIFACT_SPILL_BYTES};
...
pub struct JobDispatcher {
    ...
    layout: Option<ccb_storage::paths::PathLayout>,
...
impl JobDispatcher {
    ...
    pub fn with_layout(mut self, layout: ccb_storage::paths::PathLayout) -> Self {
    ...
    fn message_for_job(&self, job: &JobRecord) -> Option<ccb_mailbox::models::MessageRecord> {
    ...
    fn latest_child_reply(
        &self,
        edge: &CallbackEdgeRecord,
    ) -> Option<ccb_mailbox::models::ReplyRecord> {
        let control = self.mailbox_control.as_ref()?;
    ...
    fn job_for_reply(&self, reply: &ccb_mailbox::models::ReplyRecord) -> Option<JobRecord> {
        let control = self.mailbox_control.as_ref()?;
    ...
    fn decision_from_reply(
        &self,
        reply: &ccb_mailbox::models::ReplyRecord,
        child_job: &JobRecord,
    ...
    fn callback_body_summary(request: &MessageEnvelope) -> String {
        let body = Self::strip_ccb_guidance(&request.body);
    ...
    fn strip_ccb_guidance(body: &str) -> String {
        let marker = "\n\nCCB reply guidance:";
    ...
    fn mailbox_layout(&self) -> Option<&ccb_storage::paths::PathLayout> {
        self.layout.as_ref()
    }
    ...
    fn daemon_job_status_to_mailbox(status: JobStatus) -> ccb_mailbox::models::JobStatus {
        let text = format!("{:?}", status).to_lowercase();
        serde_json::from_value(serde_json::Value::String(text))
            .unwrap_or(ccb_mailbox::models::JobStatus::Incomplete)
    }
