...
use ccb_providers::execution::{ExecutionService, ExecutionStateStore, ProviderRuntimeContext};
...
impl CcbdApp {
        ...
        Self {
            ...
            completion_tracker: CompletionTrackerService::new(
                ...
                build_default_provider_catalog(
                    false,
                    std::env::var("CCB_INCLUDE_TEST_DOUBLES")
                        .map(|v| v == "1")
                        .unwrap_or(false),
                ),
                CompletionRegistry,
            ...
            ownership: OwnershipService::with_state_path(
                layout.ccbd_dir().join("ownership-state.json"),
            ),
    ...
    pub fn heartbeat(&mut self) {
        ...
        for decision in decisions {
            match decision {
                ...
                crate::supervision::loop_runner::SupervisionDecision::Escalate {
                    ...
                    eprintln!("ccbd: supervision escalated {agent_name}: {reason}");
        ...
        if !restart_agents.is_empty() {
            ...
            eprintln!(
                "ccbd: respawning agents after supervision decisions: {:?}",
                restart_agents
            );
            if let Err(e) =
                self.run_start_flow(&all_agents, false, true, config_windows, None, None, &[])
            {
                eprintln!("ccbd: respawn after supervision failed: {e}");
            }
        }
        ...
        if let Err(e) = self.respawn_dead_agents() {
            eprintln!("ccbd: respawn_dead_agents failed: {e}");
        }
        ...
        for job in running_jobs {
            ...
            match crate::tick::tick_job_heartbeat(&self.job_heartbeat, &mut adapter, &tick_job) {
                ...
                Ok(true) => {}
                Err(e) => {
                    eprintln!("ccbd: heartbeat tick failed for {}: {e}", job.job_id);
                }
            }
        }
    }
        ...
        F: FnMut(&str) -> bool,
    {
        ...
        for entry in self.registry.all_entries() {
            if let Some(pane_id) = entry.pane_id.as_deref() {
                ...
                if !is_alive(pane_id) {
                    eprintln!(
                        "ccbd: detected dead pane {} for agent {}, will respawn",
        ...
        let config_windows = self.current_config.as_ref().and_then(|c| c.windows.clone());
        eprintln!(
            "ccbd: respawning agents after pane death: {:?}",
            dead_agents
        );
        let _result =
            self.run_start_flow(&all_agents, false, true, config_windows, None, None, &[])?;
