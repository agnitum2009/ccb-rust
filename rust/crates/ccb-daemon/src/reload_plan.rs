...
fn tool_window_operations(
    current_config: &ProjectConfig,
    new_config: &ProjectConfig,
    old_tools: &HashMap<String, ccb_agents::models::ToolWindowSpec>,
    new_tools: &HashMap<String, ccb_agents::models::ToolWindowSpec>,
) -> Vec<ReloadOperation> {
    let mut operations: Vec<ReloadOperation> = Vec::new();
...
fn blocked_unsupported_operations(operations: &[serde_json::Value]) -> Vec<serde_json::Value> {
...
fn view_refresh_steps(operations: &[ReloadOperation]) -> Vec<NamespacePatchStep> {
...
fn build_namespace_topology(config: &ProjectConfig) -> Vec<TopologyWindow> {
...
fn missing_additive_agent_steps(
    operations: &[ReloadOperation],
...
fn warnings_for_status(status: &str) -> Vec<String> {
    match status {
...
fn tool_window_record_map(
    config: &ProjectConfig,
) -> HashMap<String, ccb_agents::models::ToolWindowSpec> {
    config
        .tool_windows
        .as_ref()
