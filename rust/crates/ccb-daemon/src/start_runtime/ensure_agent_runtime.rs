...
use ccb_terminal::identity::pane_visual;
...
impl EnsureAgentRuntimeImpl {
    ...
    /// Create an instance that uses the real `ccb-terminal::TmuxBackend`.
    pub fn with_default_backend(launcher: ProviderLauncher) -> Self {
        Self::new(launcher, |socket_name, socket_path| {
            Box::new(ccb_terminal::TmuxBackend::new(socket_name, socket_path))
        })
    }
...
impl EnsureAgentRuntimeFn for EnsureAgentRuntimeImpl {
    fn call(
        ...
        let binding = RuntimeBinding {
            ...
            ccb_session_id: Some(launch_session_id),
