...
use ccbr_storage::path_helpers::RootKind;
...
pub fn summarize_storage(
    layout: &PathLayout,
) -> ccbr_storage::Result<serde_json::Map<String, serde_json::Value>> {
    let entries = scan_layout(layout)?;
    Ok(summary_payload(layout, &entries))
}

fn scan_layout(layout: &PathLayout) -> ccbr_storage::Result<Vec<StorageEntry>> {
    let mut roots: Vec<(&str, Utf8PathBuf)> = vec![("project", layout.ccbr_dir())];
    if layout.runtime_state_root() != layout.ccbr_dir().as_path() {
...
fn classify_path(
    ...
    path: &Utf8Path,
    root_kind: &str,
) -> ccbr_storage::Result<StorageEntry> {
...
fn scan_identity(path: &Utf8Path) -> ccbr_storage::Result<(u64, u64)> {
...
fn unsafe_symlink_reason(path: &Utf8Path, layout: &PathLayout) -> Option<String> {
    ...
    let allowed_roots = [layout.ccbr_dir(), layout.runtime_state_root().to_path_buf()];
    if allowed_roots
        .iter()
...
fn is_allowed_provider_secret_symlink(path: &Utf8Path, layout: &PathLayout) -> bool {
    let relative = match path.strip_prefix(layout.ccbr_dir()) {
        Ok(r) => r,
        Err(_) => return false,
    };
