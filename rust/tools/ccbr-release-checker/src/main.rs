...
fn cmd_check_release_state(
    ...
    let raw_version = version.unwrap_or_else(|| {
        ccbr_release_checker::read(&root.join("VERSION"))
            .trim()
            .to_string()
    });
...
fn cmd_check_assets(repo_root: Option<Utf8PathBuf>, repo: String, version: String) -> i32 {
    ...
    let mut report = Report::default();
    ccbr_release_checker::assets::check_sha256sums(&root, &version, &repo, &mut report);
...
fn cmd_check_local(
    ...
    let version = version
        ...
        .unwrap_or_else(|| {
            normalize_version_cli(ccbr_release_checker::read(&root.join("VERSION")).trim())
        });
    let mut report = Report::default();
    check_active_skill_sync(&root, &mut report);
    check_local_git_state(&root, &phase, &mut report);
...
fn cmd_check_markdown(
    ...
    let body = ccbr_release_checker::read(&target);
