...
fn test_release_artifact_basename() {
    assert_eq!(
        ...
        Some("ccb-linux-x86_64".into())
    );
    assert_eq!(
        release_artifact_basename("linux", "arm64"),
        Some("ccb-linux-aarch64".into())
    );
    assert_eq!(
        release_artifact_basename("macos", "x86_64"),
        Some("ccb-macos-universal".into())
    );
...
fn test_release_artifact_name() {
    assert_eq!(
        ...
        Some("ccb-linux-x86_64.tar.gz".into())
    );
}
...
fn test_create_tarball_and_sha256_and_verify() {
    ...
    let artifact_root = stage_root.join("ccb-linux-x86_64");
    ...
    for name in ccb_release_builder::REQUIRED_BINARIES {
    ...
    let artifact_path = output_dir.join("ccb-linux-x86_64.tar.gz");
    ...
    assert!(sha_contents.contains("ccb-linux-x86_64.tar.gz"));
...
fn test_copy_repo_tree_respects_exclusions() {
    ...
    ccb_release_builder::copy_repo_tree(&root, &destination, &[]).unwrap();
...
fn test_export_release_tree_allow_dirty_copies_worktree() {
    ...
    ccb_release_builder::export_release_tree(&root, &destination, "HEAD", true, &[]).unwrap();
