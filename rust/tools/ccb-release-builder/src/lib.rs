...
pub fn build_sidebar_helper_for_release(artifact_root: &Path, target_platform: &str) -> Result<()> {
    let crate_dir = artifact_root.join("tools").join("ccb-agent-sidebar");
    let output_bin = artifact_root.join("bin").join("ccb-agent-sidebar");
...
fn build_macos_universal_sidebar_helper(
    ...
    if !output.status.success() {
        bail!(
            "failed to create macOS universal ccb-agent-sidebar: {}",
            String::from_utf8_lossy(&output.stderr)
        );
    }
...
fn run_sidebar_cargo_build(
    ...
    if !output.status.success() {
        bail!(
            "failed to build ccb-agent-sidebar{} for release:\n{}",
            target
...
fn verify_macos_universal_sidebar_binary(output_bin: &Path) -> Result<()> {
    ...
    if !output.status.success() {
        bail!(
            "failed to inspect macOS ccb-agent-sidebar binary: {}",
            String::from_utf8_lossy(&output.stderr)
        );
    }
    ...
    if !description.contains("universal binary") {
        bail!(
            "macOS ccb-agent-sidebar is not a universal binary: {}",
            description.trim()
        );
    }
    Ok(())
