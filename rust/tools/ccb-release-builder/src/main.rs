...
use ccb_release_builder::{BuildOptions, PackageOptions, VerifyOptions};

#[derive(Parser)]
#[command(name = "ccb-release-builder")]
...
enum Commands {
    ...
    Build {
        ...
        #[arg(long, value_name = "DIR", default_value = ccb_release_builder::DEFAULT_OUTPUT_DIR)]
        output_dir: PathBuf,
...
fn run() -> Result<()> {
    ...
    match cli.command {
        Commands::Build {
            ...
            ccb_release_builder::run_build(&opts)?;
        }
        Commands::Package {
            platform,
            artifact_dir,
            ...
            ccb_release_builder::run_package(&opts)?;
        }
        Commands::Verify { artifact } => {
            let opts = VerifyOptions {
                artifact_path: artifact,
            };
            ccb_release_builder::run_verify(&opts)?;
        }
    }
    Ok(())
}
