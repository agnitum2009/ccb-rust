//! Integration tests for the CCB MCP delegation server.
//!
...
use ccb_mcp_server::{DaemonClient, HandleOutcome, McpRequest};
...
fn unwrap_text_payload(resp: &ccb_mcp_server::McpResponse) -> Value {
...
fn test_tools_list_returns_three_definitions() {
    let tools = ccb_mcp_server::tool_definitions();
    ...
    let pend = by_name["ccb_pend_agent"];
    assert_eq!(
        pend["inputSchema"]["required"],
        serde_json::json!(["target"])
    );

    let ping = by_name["ccb_ping_agent"];
...
fn test_ask_agent_dispatches_submit() {
    ...
    let outcome =
        ccb_mcp_server::handle_request_with_factory(req, "droid", fake_factory(client.clone()));
    ...
    let layout = ccb_storage::paths::PathLayout::new(FAKE_PROJECT_ROOT);
...
fn test_ping_agent_defaults_to_ccbd() {
    let client = FakeClient::with_response(
        "ping",
        serde_json::json!({ "pong": true, "target": "ccbd" }),
    );

    let req = tools_call_request(5, "ccb_ping_agent", serde_json::json!({}));
...
fn test_unknown_tool_returns_error() {
    ...
    let req = tools_call_request(6, "ccb_no_such_tool", serde_json::json!({}));
