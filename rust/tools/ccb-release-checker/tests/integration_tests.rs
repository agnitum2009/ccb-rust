...
fn bin_path() -> std::path::PathBuf {
    std::env::var_os("CARGO_BIN_EXE_ccb-release-checker")
        .map(std::path::PathBuf::from)
        .unwrap_or_else(|| {
            repo_root()
                .join("target/debug/ccb-release-checker")
                .into_std_path_buf()
        })
}
...
fn test_cli_help() {
    ...
    assert!(stdout.contains("ccb-release-checker"));
...
"#;
    assert_eq!(
        ccb_release_checker::markdown::markdown_section(body, "v1.0.0 (2024-01-01)"),
...
"#,
    ...
    let mut report = ccb_release_checker::Report::default();
    ccb_release_checker::local::check_local_files(
        &root,
        "v1.0.0",
...
fn test_workflow_wait_status() {
    use ccb_release_checker::workflows::format_workflow_wait_status;
