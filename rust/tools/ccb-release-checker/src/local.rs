...
pub fn check_active_skill_sync(root: &Utf8Path, report: &mut Report) {
    let source_dir = root.join("dev_tools/skills/ccb-github");
    ...
    let pattern =
        ccb_dir.as_str().to_string() + "/agents/*/provider-state/codex/home/skills/ccb-github";
    ...
    for active_dir in glob_results {
        ...
        if !mismatched.is_empty() {
            report.warn(format!(
                "Active ccb-github skill copy differs from dev_tools at {active_utf8}: {}",
                mismatched.join(", ")
            ));
        }
    }
