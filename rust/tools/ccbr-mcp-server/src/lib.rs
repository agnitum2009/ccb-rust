...
//!   handler function.
//! - `ccbr_pend_agent` uses the daemon's `get` RPC (job_id or agent_name)
//!   instead of the Python `cli.services.pend.pend_target`, which had no Rust
//!   equivalent.
//! - `ccbr_ask_agent` computes `project_id` from the resolved project root via
//!   `ccbr_storage::paths::PathLayout` rather than from a Python CLI context.
...
pub const SERVER_NAME: &str = "ccbr-delegation";
...
impl<T: ccbr_cli::services::DaemonClient> DaemonClient for T {
    fn call(&self, method: &str, params: Value) -> Result<Value, String> {
        ccbr_cli::services::DaemonClient::call(self, method, params)
    }
}
...
pub fn build_client(
    work_dir: Option<&str>,
) -> Result<(PathBuf, ccbr_cli::services::UnixDaemonClient), String> {
    ...
    let project_root = ccbr_cli::services::resolve_project_root(&project_root, None)?;
    let socket_path = ccbr_cli::services::socket_path_for_project(&project_root);
    Ok((
        project_root,
        ccbr_cli::services::UnixDaemonClient::new(socket_path),
    ))
}
    ...
    F: FnMut(Option<&str>) -> Result<(PathBuf, Box<dyn DaemonClient>), String>,
{
    ...
    let result = match name.as_str() {
        "ccbr_ask_agent" => ask_agent(&args, caller, &_project_root, client.as_ref()),
        "ccbr_pend_agent" => pend_agent(&args, client.as_ref()),
        "ccbr_ping_agent" => ping_agent(&args, client.as_ref()),
...
mod tests {
    ...
    fn test_tools_list_returns_three_tools() {
        ...
        assert_eq!(
            names,
            vec!["ccbr_ask_agent", "ccbr_pend_agent", "ccbr_ping_agent"]
        );
    }
