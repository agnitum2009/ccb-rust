...
fn hook_bin() -> PathBuf {
    PathBuf::from(env!("CARGO_BIN_EXE_ccb-provider-activity-hook"))
}
...
fn test_provider_activity_hook_writes_codex_active_snapshot() {
    ...
    env.insert("CCB_SESSION_ID".into(), "ccb-agent2-1".into());
    ...
    assert_eq!(activity["ccb_session_id"], "ccb-agent2-1");
