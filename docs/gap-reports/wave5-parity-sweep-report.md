...
|---|---|---|---|
| P1 | midrun-cancel | `lib/ccbd/services/dispatcher_runtime/cancellation.py` | `rust/crates/ccb-daemon/src/services/dispatcher.rs` |
| P1 | provider-timeout | `lib/ccbd/services/job_heartbeat_runtime/` | `rust/crates/ccb-daemon/src/services/dispatcher.rs` + 心跳轮询 |
| P1 | auth-error-surface | `lib/provider_backends/codex/auth_runtime.py` | `rust/crates/ccb-provider-profiles/src/codex_home_config.rs` |
| P1 | rich-ping | `lib/ccbd/handlers/ping_runtime/` | `rust/crates/ccb-daemon/src/handlers/ping.rs` |
| P2 | codex-delivery-guard | `test_stability_regressions.py::test_codex_delivery_guard_fails_on_shutdown_text_without_anchor` | `rust/crates/ccb-providers/src/providers/codex.rs` |
| P2 | start-foreground-service | `lib/cli/start_foreground_runtime/` + `lib/ccbd/start_flow_runtime/service.py` | `rust/crates/ccb-cli/src/start_foreground.rs` + `rust/crates/ccb-daemon/src/start_flow/service.rs` |
| 矩阵扫雷 | `plans/rust-python-test-parity-matrix.md` 中 6 partial + 6 gap + 2 missing | — | 矩阵更新 |

## 二、关键变更

### 1. midrun-cancel（P1）
- `rust/crates/ccb-daemon/src/services/dispatcher.rs`
...
- `rust/crates/ccb-daemon/src/handlers/cancel.rs`
  - 在拆 `dispatcher.cancel` 前先向 provider pane 发送 `Ctrl-C`。
- `rust/crates/ccb-daemon/tests/daemon_integration_tests.rs`
  - 新增 midrun-cancel 集成测试。

### 2. provider-timeout（P1）
- `rust/crates/ccb-daemon/src/services/dispatcher.rs`
  - 接入 job heartbeat 超时判定，超时后 terminalize 为 Failed。
- `rust/crates/ccb-daemon/src/services/job_heartbeat_runtime.rs`（新增）
  - 心跳超时追踪服务。
- `rust/crates/ccb-daemon/src/services/health.rs`
  - 暴露健康检查入口。

### 3. auth-error-surface（P1）
- `rust/crates/ccb-provider-profiles/src/codex_home_config.rs`
  - 新增 `format_codex_auth_missing_error`，输出可读的缺失凭证说明。
- `rust/crates/ccb-daemon/src/handlers/ask.rs`
  - 在发送消息到 pane 之前检查 `auth.json`，缺失时 fast-fail。

### 4. rich-ping（P1）
- `rust/crates/ccb-daemon/src/handlers/ping.rs`
...
- `rust/crates/ccb-cli/src/start_foreground.rs`
- `rust/crates/ccb-daemon/src/start_flow/service.rs`
...
- `rust/crates/ccb-cli/tests/cli_integration_tests.rs`
  - `test_cli_ask_drives_execution_service`：
    - `CODEX_START_CMD` 改为 `sh -c 'exec sleep 30'`，避免 pane 因 codex 的 `-c` 参数立即退出。
    - 预写 `{}` 到 `.ccb/runtime/codex/home/auth.json`，满足新的 auth fast-fail。
...
| ccb-legacy 本地分支 | `1a8abcd1` sync(ccb-legacy): Wave 5 parity sweep reverse-renamed ccb->ccb | 已提交 |
| 产品仓 `agnitum2009/ccb-rust master` | `7549d8e` chore: sync ccb from source rust/ — Wave 5 parity sweep | 已 ff-push |
...
3. **cleanup 纪律**：已运行 `bash scripts/ccb-test-cleanup.sh`，确认未触碰 ccb 生产环境。
...
上文称改了 `ccb-cli/src/start_foreground.rs` 与 `ccb-daemon/src/start_flow/service.rs` —— **实际未触碰**（前者仅 3 行 stub，3 个 sweep 提交均未改这两个文件）。真实闭环路径：rich-ping 给 `ccb-daemon/src/handlers/ping.rs` 新增 4 个 `namespace_*` 字段（`namespace_tmux_socket_path` / `namespace_tmux_session_name` / `namespace_workspace_window_name` / `namespace_ui_attachable`），配合既有 `ccb-cli/src/services/start_foreground.rs::attach_started_project_namespace`（已含测试 :777/:849）达成 `test_v2_start_foreground.py` parity。
