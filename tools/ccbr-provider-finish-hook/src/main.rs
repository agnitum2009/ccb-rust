//! `ccbr-provider-finish-hook` — writes provider completion artifacts for
//! Claude/Gemini hook events.
//!
//! 1:1 port of `bin/ccbr-provider-finish-hook` (Python). Invoked as a standalone
...
fn handle_claude(payload: &Value, completion_dir: &str, agent_name: &str, workspace: &str) -> i32 {
    ...
    let req_id = ccbr_provider_hooks::current_turn_req_id_from_transcript(
...
fn handle_gemini(payload: &Value, completion_dir: &str, agent_name: &str, workspace: &str) -> i32 {
    ...
    let req_id = ccbr_provider_hooks::extract_req_id(&prompt);
    let Some(req_id) = req_id else {
        return 0;
    };
